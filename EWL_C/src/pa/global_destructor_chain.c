 ...
 *	THEORY OF OPERATION
 *
 *	When the compiler generates code to call initializers for global
 *variables and nonlocal static variables, it calls the function
 *__register_global_object() to register the constructed object for
 *destruction.
 *
 *	When the program exits, it calls __destroy_global_chain() to call
 *destructors for the registered objects, in the reverse order of their
 *construction.
 *
 *	For most applications, there is a single global destructor chain for the
 *	entire program, even if the program is composed of multiple fragments
 *(shared libs) This is necessary to support the proper program termination
 *sequence, wherein all destructors must be called before e.g. closing open
 *files and cleaning up the console window.
 *
 *	"Drop-ins" which may load and unload as a program is running should each
 *have their own global destructor chain which is created when the drop-in is
 *loaded and destroyed when the drop-in is unloaded.
 *
 */
...
__declspec(section ".dtors$10") __declspec(weak) extern void *const

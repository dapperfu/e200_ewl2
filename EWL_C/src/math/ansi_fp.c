 ...
 *	Routines
 *	--------
 *		These are routines similar to those from MathLib needed by non-Mac
 *systems. Note that we assume 8-byte doubles here.
 *
 *			__num2dec -	same as num2dec, except that we don't
 *support the FIXEDDECIMAL style result (all conversions are handled in printf
 *using the FLOATDECIMAL output)
 *
...
#define ldexp(x, y) (f64_t)(ldexpf)((f32_t)(x), (y))
#define frexp(x, y) (f64_t)(frexpf)((f32_t)(x), (y))
#define copysign(x, y) (f64_t)(copysignf)((x), (y))
#define pow(x, y) (f64_t)(powf)((x), (y))
#endif /* if _EWL_DOUBLE_SIZE == 32 */
...
static f64_t copysign(f64_t x, f64_t y) {
  GET_DOUBLE_UHI_WORD(x) = (GET_DOUBLE_UHI_WORD(x) & 0x7fffffffu) |
                           (GET_DOUBLE_UHI_WORD(y) & 0x80000000u);
  return x;
}
...
#error unsupported sizeof double
  ...
  /*
          There is an internal format and external format for the decimal
     structure. The external format is output from num2dec, and input for
     dec2num. The internal format is used everywhere else. The external format
     uses ASCII for the digits, and the decimal point is to the right of the
     least significant digit. The internal format uses numerical values for the
     digits ('\0' is 0) and the decimal point is to the right of the most
     significant digit.
  */
...
static void __test_preconditions() {
...
#endif /* _TEST */
    int_t
    __ewl_count_trailing_zero_double(f64_t x) {
#if _DOUBLE_BITS == 64
  MISRA_EXCEPTION_RULE_1_2c() MISRA_EXCEPTION_RULE_11_4() uint64_t *l =
      (uint64_t *)&x;
  MISRA_EXCEPTION_RULE_10_1()
  return (int_t)__ewl_count_trailing_zero64(*l |
                                            (uint64_t)0x0010000000000000ULL);
#elif _DOUBLE_BITS == 32
...
#endif /* _TEST */
    int_t
    __must_round(const decimal *d, int_t pos) {
...
#endif /* _TEST */
    void
    __dorounddecup(decimal *d, int_t digits) {
  char_t *b = d->sig.text;
  char_t *i = b + (digits - 1);
  for (;;) {
    if (*i < 9) {
...
#endif /* TEST */
    void
    __rounddec(decimal *d, int_t digits) {
  int_t r;
...
#endif /* _TEST */
    void
    __str2dec(decimal *d, const char_t *s, int16_t xp) {
  int_t i;
  d->exp = xp;
  d->sgn = 0;
...
static
#endif /* _TEST */
    void
    __two_exp(decimal *result, int32_t xp) {
  ...
  }
  {
    ...
    __two_exp(&x2, ((int32_t)((0x80000000UL & (uint32_t)xp) >> 31) + xp) >>
                       1); /*__two_exp(&x2, exp/2); */
  ...
  if ((jn - jb) < (int_t)y->sig.length) {
    ...
    if (*jn < 5) {
      ...
      if (*jn == 5) {
        ...
        i = (ib + (jn - jb) + dexp) - 1;
        MISRA_EXCEPTION_RULE_12_7a() if (*i & 1) { round_down = 1; }
      }
    }
    if (round_down) {
      if (*i < 1) {
...
static
#endif /* _TEST */
    void
    __num2dec_internal(decimal *d, f64_t x) {
  ...
  }
  {
    ...
    int_t num_bits_extract =
        DBL_MANT_DIG - __ewl_count_trailing_zero_double(frac);
...
f64_t __dec2num(const decimal *d) {
  ...
  case 'N': {
    f64_t result;
    MISRA_EXCEPTION_RULE_11_4()
    MISRA_EXCEPTION_RULE_1_2c() d_int *ll = (d_int *)&result;
#if _DOUBLE_BITS == 64
...
#error __dec2num not supported
  ...
  }
  {
    ...
    exponent = dec.exp;
    { /* Check for numbers greater than the half-way point between DBL_MAX and
         INFINITY*/
      decimal mx;
#if _DOUBLE_BITS == 64
...
#error __dec2num not supported
    ...
    }
    {
      ...
      f64_t next_guess;
      MISRA_EXCEPTION_RULE_11_4()
      MISRA_EXCEPTION_RULE_1_2c() d_int *ull = (d_int *)&next_guess;
      int_t guessed_low = 0;
      ...
      if (__equals_dec(&difflow, &diffhigh)) {
        ...
        MISRA_EXCEPTION_RULE_11_4()
        MISRA_EXCEPTION_RULE_1_2c() if (*(d_int *)&first_guess & 1u) {
          first_guess = next_guess;
        }
      } else {
        if (!__less_dec(&difflow, &diffhigh)) {
          first_guess = next_guess;
        }
      }
    }
...
static const f64_t bit_values[] = {1.0E+001, 1.0E+002, 1.0E+004,
                                   1.0E+008, 1.0E+016, 1.0E+032,
                                   1.0E+064, 1.0E+128, 1.0E+256};

#define max_bits                                                               \
  511L /* (2**n)-1 where n is the number of elements in bit_values */

static const f64_t digit_values[] = {1.0E+01, 1.0E+02, 1.0E+03, 1.0E+04,
...
void __num2dec(const decform *f, f64_t x, decimal *d) {
  ...
  /*
   *	Next, we use frexp() to separate x into a fraction f and an exponent e.
   *The fraction is a value between 0.5 and 1.0 such that f * (2**e) == x. This
   *is basically how x is stored internally. All we're interested in is the
   *	exponent to approximate the factor of 10 by which to scale x to get rid
   *of its exponent.
   */
  ...
  /*
   *	We convert the base-2 exponent to a base-10 exponent by multiplying it
   *by the base-10 logarithm of 2 (approximately .30102999566398). Rather than
   *	invoke the overhead of conversion to floating point and back, we instead
   *	multiply by 301029 and then divide by 1000000. The largest double
   *exponent is 1024; 301029 * 1024 == 308253696, which can easily be
   ...
   *	multiply or divide by 10, exp10 times. However, as noted above, the
   *largest binary exponent is 1024. It's decimal equivalent is 1024 log 2 ≈≈
   *308. That's potentially a lot of multiplies/divides. Alternatively, we can
   *scan the decimal exponent bitwise and, for each bit 2**i, scale by a
   *constant 10**(2**i) if that bit is nonzero. This results in at most nine
   *multiplies or divides.
   *
   *	Note that division usually consumes more cycles than multiplication.
   *	Therefore, we avoid doing multiple division operations by multiplying
   *the scale factors together and then dividing only once.
   */
  ...
  /*
   *	Now we scale up or down as necessary to ensure that x is >= 0.1 and
   *< 1.0. This puts all the digits to the right of the decimal point.
   */
  ...
  /*
   *	Now we pick off decimal digits from left to right up to eight at a time.
   *We do this by shifting digits to the left of the decimal point and
   *converting to long int to extract the shifted digits. We then decode the
   *long int in the usual way.
   */
...
f64_t __dec2num(const decimal *d) {
  ...
  */
  ...
  /*
   *	We're going to accumulate digits eight at a time (enough to fit into a
   *long int). If the number of digits isn't evenly divisible by eight, the
   *stragglers are on the left as far as we are concerned.
   */

  {
    ...
    /*
     *	Once we're done we're going to shift the decimal point from the far
     *right to the immediate right of the most significant digit. We make some
     *adjustments here toward that end.
     */
...

  {
    ...
    */
    /*	letting "*" and "/" handle "inf" and "0.0" situations seems to work much
     * better */
    MISRA_EXCEPTION_RULE_12_7()
    for (; n; p++, (n >>= 1L)) {
      if (n & 1L) {

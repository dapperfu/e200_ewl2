...
#else
f64_t __ieee754_asin(x) f64_t x;
#endif
{
  ...
  if (ix >= 0x3ff00000L) { /* |x|>= 1 */
    MISRA_EXCEPTION_RULE_12_7()
    if (((ix - 0x3ff00000L) | GET_DOUBLE_LO_WORD(x)) ==
        0L) { /* asin(1)=+-pi/2 with inexact */
  ...
  } else if (ix < 0x3fe00000L) { /* |x|<0.5 */
    if (ix < 0x3e400000L) {      /* if |x| < 2**-27 */
      if ((big + x) > one) {
        MISRA_EXCEPTION_RULE_14_7()
        return x;

...
extern float fmodf(float x, float y) {
  ...
  // and return x-Q*y
                  ...
                  */
  _UINT32 exp_shift;
  _UINT32 exp_diff, quotient = 0, scaled_x,
                    scaled_y; /*quotient=quotient_hi+quotient_lo */
  ...
  exp_diff = ((GET_DOUBLE_UHI_WORD(x) & 0x7f800000uL) -
              (GET_DOUBLE_UHI_WORD(y) & 0x7f800000uL)) >>
             23;
  exp_shift = exp_diff &
              0x0000001f; /* shift at most 31 to insure quotient is < 2^^32 -1
  ...
  __asm
  {
    ...
    mov edx,scaled_x
    mov ecx,scaled_y
    shl ecx,1 // multiply divisor by two, to avoid overflow(i.e. to insure quotient is <
      // 2^^32 -1 )
    div ecx
    mov quotient, eax
  }
...
#endif
  quotient >>= (31 - exp_shift); /* truncates instead of round to nearest(this
                                 is correct since we want
                                 // largest integer such that (x-Q*y < y).
                                 Round to nearest may produce one too large.
                                 // multiply quotient term by 2^^exp_diff , if
                                 exp_diff > 32 this product will be rounded and
                                 // will not be affected by the least
                                 significant bits of the original y. */

  GET_DOUBLE_HI_WORD(z) -= exp_diff;
  GET_DOUBLE_HI_WORD(y_hi) =
      GET_DOUBLE_HI_WORD(y) &
      0x7ffff000; /* the division of y into two floats(y_lo,y_hi) w/ at most 12
significant bits
...
// still this quotient is inadequate for dividend/divisor pairs whose exponents
differ
// by more than 32 */
  ...
  z = fabsf((float)((((y_hi * f_quotient[2] - z) + y_hi * f_quotient[1]) +
                     y_hi * f_quotient[0]) +
                    (y_lo * f_quotient[2] +
                     (y_lo * f_quotient[1] + y_lo * f_quotient[0]))));

...
__RCSID("$NetBSD: s_erff.c,v 1.5 1997/10/09 11:31:38 lukem Exp $");
#endif
...
f32_t erff(f32_t x)
#else
f32_t erff(x) f32_t x;
#endif
{
  int32_t hx, i;
  uint32_t ix;
  f32_t R, S, P, Q, s, y, z, r;

  GET_FLOAT_WORD(hx, x);
  ix = (uint32_t)hx & 0x7fffffffUL;
  if (ix >= 0x7f800000UL) { /* erf(nan)=nan */
    MISRA_EXCEPTION_RULE_10_3()
    i = (int32_t)(((uint32_t)hx >> 31) << 1);
    i = 1 - i;
    MISRA_EXCEPTION_RULE_14_7()
    return ((f32_t)i + (one / x)); /* erf(+-inf)=+-1 */
  }

  if (ix < 0x3f580000UL) {   /* |x|<0.84375 */
  ...
  if (ix < 0x3fa00000UL) { /* 0.84375 <= |x| < 1.25 */
    s = fabsf(x) - one;
    P = pa0 +
        (s *
         (pa1 +
  ...
  SET_FLOAT_UWORD(z, ix & 0xfffff000UL);
  MISRA_EXCEPTION_RULE_1_2b() r = __ieee754_expf((-z * z) - 0.5625F) *
...
#endif
{
  ...
  GET_FLOAT_WORD(hx, x);
  ix = (uint32_t)hx & 0x7fffffffUL;
  if (ix >= 0x7f800000UL) { /* erfc(nan)=nan */
    /* erfc(+-inf)=0,2 */
    MISRA_EXCEPTION_RULE_10_3()
    MISRA_EXCEPTION_RULE_14_7()
    return ((f32_t)(((uint32_t)hx >> 31) << 1) + (one / x));
  }

  if (ix < 0x3f580000UL) {   /* |x|<0.84375 */
    if (ix < 0x23800000UL) { /* |x|<2**-56 */
      MISRA_EXCEPTION_RULE_14_7()
      return one - x;
    }
    z = x * x;
  ...
  if (ix < 0x41e00000UL) { /* |x|<28 */
    x = fabsf(x);
    s = one / (x * x);
    if (ix < 0x4036DB6DUL) { /* |x| < 1/.35 ~ 2.857143*/
      R = ra0 +
          (s *
           (ra1 +
            (s *
          ...
          (s *
           (sb1 +
            (s *
             (sb2 +
              (s *
               (sb3 + (s * (sb4 + (s * (sb5 + (s * (sb6 + (s * sb7)))))))))))));
    }
    GET_FLOAT_UWORD(ix, x);
    SET_FLOAT_UWORD(z, ix & 0xfffff000UL);
    MISRA_EXCEPTION_RULE_1_2b() r =
        __ieee754_expf((-z * z) - 0.5625F) *
        __ieee754_expf(((z - x) * (z + x)) + (R / S));
    if (hx > 0L) {
      MISRA_EXCEPTION_RULE_14_7()
      return r / x;
...
#if _EWL_C99
    if (x >= HUGE_VALF) { /* range error may occur */

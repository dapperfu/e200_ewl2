...
#else
f32_t __kernel_tanf(x, y, iy) f32_t x, y;
int32_t iy;
#endif
{
  f32_t z, r, v, w, s;
  ...
  if (iy == 1L) {
    ...
    z = w;
    GET_FLOAT_WORD(i, z);
    MISRA_EXCEPTION_RULE_12_7a() SET_FLOAT_WORD(z, i & 0xfffff000);
    v = r - (z - x);       /* z+v = r+x */
    t = (a = -(1.0F / w)); /* a = -1.0/w */
    GET_FLOAT_WORD(i, t);
    MISRA_EXCEPTION_RULE_12_7a() SET_FLOAT_WORD(t, i & 0xfffff000);
    s = 1.0F + (t * z);
    MISRA_EXCEPTION_RULE_14_7()
    return t + (a * (s + (t * v)));
  }

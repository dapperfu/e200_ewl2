...
#if _EWL_FLOATING_POINT && !__has_intrinsic(__builtin_fma) &&                  \
    _EWL_USES_SUN_DP_MATH_LIB && _EWL_LONGLONG
...
_EWL_IMP_EXP_C f64_t _EWL_MATH_CDECL fma(f64_t x, f64_t y, f64_t z) {
  ...
  if ((x == 0.0) || (ex == 0x7FF00000L) || (y == 0.0) || (ey == 0x7FF00000L) ||
  ...
  /* all of x, y and z are now either FP_NORMAL or FP_SUBNORMAL
     represent each x as:  sx = sign, ex = biased exponent, x_hi:x_lo 53 bit
     mantissa shift exponents down, they are still biased */
  MISRA_EXCEPTION_RULE_12_7() {
    ex >>= 20;
    ey >>= 20;
    ez >>= 20;
  }
  /* mask off mantissas.  Explicitly store implicit leading bit.  And correct 0
   * exponent */
  x_hi &= 0x000FFFFFUL;
  if (ex == 0L) {
    ex = 1;
  ...
  /* Perform p = x * y
     p will be 118 bits stored in 2 uint64_t's, p_hi:p_lo (only 106 bits are
     actually generated) compute least significant 32 bits of p_lo.  Use p_hi as
     an accumulator. */
  ...
  if (shift > -106L) { /* if z is big enough so that it can effect p */
    ...
    if (shift > 0L) {
      ...
      MISRA_EXCEPTION_RULE_10_3()
      xp =
          (uint32_t)(((0xFFFFFFFFFFFFFFFFULL >> (64L - shift)) & p_lo) != 0ULL);
      ...
      if (shift < 0L) {
        ...
        if (shift < 64L) {
          ...
          MISRA_EXCEPTION_RULE_10_3()
          xq = (uint32_t)(((0xFFFFFFFFFFFFFFFFULL >> (128L - shift)) & q_hi) !=
                          0ULL);
          q_lo = q_hi >> (shift - 64L);
          q_hi = 0U;
        }
    ...
    if (sp ^ sz) { /* subtract */
      ...
      if ((high_sign < 0L) || ((high_sign == 0L) && (low_sign < 0L)) ||
  ...
  case 1: /* to nearest (round to even on tie) */
    if ((p_lo & 0x8000000000000000ULL) &&
        ((p_hi & 1ULL) || (p_lo & 0x7FFFFFFFFFFFFFFFULL) || xp)) {
      ++p_hi;
    }
    break;
  case 2: /* towards inf */
...
#endif /* _EWL_FLOATING_POINT && _EWL_USES_SUN_DP_MATH_LIB && _EWL_LONGLONG */

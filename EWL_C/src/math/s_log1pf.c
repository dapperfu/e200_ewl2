...
#endif
{
...
#endif
        MISRA_EXCEPTION_RULE_1_2d() MISRA_EXCEPTION_RULE_14_7() return -two25 /
            zero; /* log1p(-1)=+inf */
      } else {
#if _EWL_C99
        MISRA_EXCEPTION_RULE_13_7()
  ...
  if (k == 0L) {
    ...
    MISRA_EXCEPTION_RULE_14_7()
    return ((f32_t)k * ln2_hi) -
           ((hfsq - ((s * (hfsq + R)) + (((f32_t)k * ln2_lo) + c))) - f);
  }
}

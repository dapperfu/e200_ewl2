...
_EWL_IMP_EXP_C f64_t _EWL_MATH_CDECL nexttoward(f64_t x, ldbl_t y) {
  ...
  MISRA_EXCEPTION_RULE_1_2c() MISRA_EXCEPTION_RULE_11_4() uint32_t *lx =
      (uint32_t *)&x;
  MISRA_EXCEPTION_RULE_1_2c() MISRA_EXCEPTION_RULE_11_4() uint32_t *ly =
      (uint32_t *)&y;

#if _EWL_LITTLE_ENDIAN
  uint32_t x_exp = lx[1] & 0x7FF00000;
  if ((((ly[1] & 0x7FF00000u) == 0x7FF00000u) &&
       ((ly[1] & 0x000FFFFFu) | ly[0])) ||
      (x == y)) {
  ...
  if ((((ly[0] & 0x7FF00000u) == 0x7FF00000u) &&
       ((ly[0] & 0x000FFFFFu) | ly[1])) ||
      (x == y)) {
    MISRA_EXCEPTION_RULE_14_7()
    return (f64_t)y;
  }
  ...
  if ((x_exp != 0x7FF00000u) || (increase != positive)) {
    MISRA_EXCEPTION_RULE_1_2c() MISRA_EXCEPTION_RULE_11_4() uint64_t *llx =
        (uint64_t *)&x;
    if (x == 0.0) {
      if (increase) {

...
#else
f64_t __ieee754_atan2(y, x) f64_t y, x;
#endif
{
  f64_t z;
  ...
  if ((((uint32_t)ix | ((lx | (uint32_t) - (int32_t)lx) >> 31)) >
       0x7ff00000UL) ||
      (((uint32_t)iy | ((ly | (uint32_t) - (int32_t)ly) >> 31)) >
       0x7ff00000UL)) { /* x or y is NaN */
...
#if _EWL_C99
  if ((((uint32_t)iy | ly) == 0u) &&
      (((uint32_t)ix | lx) == 0u)) { /* y==0 && x==0; domain error may occur */

...
#else
f64_t __ieee754_pow(x, y) f64_t x, y;
#endif
{
  ...
  if (iy > 0x41e00000UL) {   /* if |y| > 2**31 */
    ...
    /* t_h=ax+bp[k] High */
    t_h = zero;
    GET_DOUBLE_UHI_WORD(t_h) =
        ((ix >> 1) | 0x20000000UL) + 0x00080000UL + ((uint32_t)k << 18);
    t_l = ax - (t_h - bp[k]);
  ...
  if (j >= 0x40900000L) {                                     /* z >= 1024 */
    ...
    if (((uint32_t)j & 0x7fffffffUL) >= 0x4090cc00UL) { /* z <= -1075 */
      if (((uint32_t)(j - (int32_t)0xc090cc00L) | (uint32_t)i) !=
          0UL) { /* z < -1075 */
        MISRA_EXCEPTION_RULE_14_7()

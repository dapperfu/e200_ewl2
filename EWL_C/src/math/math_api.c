...
int_t _EWL_MATH_CDECL __signbitf(f32_t x) {
  MISRA_EXCEPTION_RULE_1_2c() MISRA_EXCEPTION_RULE_11_4() int_t res =
      (((*(uint32_t *)&x) & 0x80000000UL) != 0UL);
  return res;
}

int_t _EWL_MATH_CDECL __fpclassifyf(f32_t x) {
  MISRA_EXCEPTION_RULE_11_4()
  MISRA_EXCEPTION_RULE_1_2c() switch ((*(uint32_t *)&x) & 0x7f800000UL) {
  case 0x7f800000:
...
int_t _EWL_MATH_CDECL __fpclassifyd(f64_t x) {
  ...
  case 0x7ff00000L:
    if ((GET_DOUBLE_UHI_WORD(x) & 0x000fffffUL) ||
        (GET_DOUBLE_LO_WORD(x) != 0L)) {
      MISRA_EXCEPTION_RULE_14_7()
      return FP_NAN;
    } else {
      MISRA_EXCEPTION_RULE_14_7()

...
#else
static f32_t sin_pif(x) f32_t x;
#endif
{
  f32_t y, z;
  int32_t n;
  ...
  case 3:
  case 4:
    y = __kernel_sinf(pi * (one - y), zero, 0L);
    break;
  case 5:
  case 6:
    ...
    break;
  default:
    y = __kernel_sinf(pi * (y - 2.0F), zero, 0L);
    break;
  }
  return -y;
}
...
#else
f32_t __ieee754_lgammaf_r(x, signgamp) f32_t x;
int_t *signgamp;
#endif
{
  ...
  *signgamp = 1;
  ...
  if (ix == 0UL) {
    MISRA_EXCEPTION_RULE_1_2d() MISRA_EXCEPTION_RULE_14_7() return one / zero;
  }

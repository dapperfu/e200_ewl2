...
MISRA_QUIET_LINT_03()
#ifdef __STDC__
static const f64_t one = 1.0, Zero[] = {
                                  0.0,
                                  -0.0,
};
#else
static f64_t one = 1.0, Zero[] = {
                            0.0,
                            -0.0,
};
#endif
...
#else
f64_t __ieee754_fmod(x, y) f64_t x, y;
#endif
{
  ...
  if ((((uint32_t)hy | ly) == 0UL) ||
      (hx >= 0x7ff00000L) || /* y=0,or x not finite */
      (((uint32_t)hy | ((ly | (uint32_t) - (int32_t)ly) >> 31)) >
       0x7ff00000UL)) { /* or y is NaN */

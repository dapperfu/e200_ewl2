...
int_t __wpformatter(void *(_EWL_CDECL *wWriteProc)(void *a, const wchar_t *b,
                    ...
                    int_t is_secure);
void *_EWL_CDECL __wFileWrite(void *File, const wchar_t *Buffer,
                              size_t NumChars);
...
int_t _EWL_CDECL vwprintf(const wchar_t *_EWL_RESTRICT format, va_list arg) {
  int_t retval;
  if (fwide(stdout, 1) <= 0) {
  ...
  retval = __wpformatter(__wFileWrite, (void *)stdout, format, arg, 0);

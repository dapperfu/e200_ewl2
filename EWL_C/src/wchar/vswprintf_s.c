...
void *_EWL_CDECL __wStringWrite(void *wosc, const wchar_t *Buffer,
                                size_t NumChars);
...
int_t _EWL_CDECL vswprintf_s(wchar_t *_EWL_RESTRICT s, rsize_t n,
                             const wchar_t *_EWL_RESTRICT format, va_list arg) {
  int_t retval;

  retval = vsnwprintf_s(s, n, format, arg);

  if ((retval >= 0) && (n > 0U) && (n < (rsize_t)RSIZE_MAX) &&
      ((rsize_t)retval >= n)) {
...
int_t _EWL_CDECL vsnwprintf_s(wchar_t *_EWL_RESTRICT s, rsize_t n,
                              const wchar_t *_EWL_RESTRICT format,
                              va_list arg) {
  int_t count;
  __wOutStrCtrl wosc;

  if ((s == NULL) || (format == NULL) || (n == 0U) ||
      (n > (rsize_t)RSIZE_MAX)) {
    __ewl_runtime_constraint_violation_s(NULL, NULL, -1);

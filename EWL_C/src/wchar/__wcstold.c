 ...
 *		exp_digit_loop
 *
 *			Process digits from the exponent. We watch for short int
...
#else
static const uint_t start = 0x0001u, sig_start = 0x0002u,
                    leading_sig_zeroes = 0x0004u, int_digit_loop = 0x0008u,
                    frac_start = 0x0010u, frac_digit_loop = 0x0020u,
                    sig_end = 0x0040u, exp_start = 0x0080u,
                    leading_exp_digit = 0x0100u, leading_exp_zeroes = 0x0200u,
                    exp_digit_loop = 0x0400u, finished = 0x0800u,
                    failure = 0x1000u,
#if _EWL_C99
                    nan_state = 0x2000u, infin_state = 0x4000u,
#endif
                    hex_state = 0x8000u;

static const uint_t not_hex = 0x0000u, hex_start = 0x0001u,
                    hex_leading_sig_zeroes = 0x0002u,
                    hex_int_digit_loop = 0x0004u, hex_frac_digit_loop = 0x0008u,
                    hex_sig_end = 0x0010u, hex_exp_start = 0x0020u,
...
ldbl_t _EWL_CDECL __wcstold(int_t max_width,
  ...
  wchar_t radix_marker =
      (wchar_t)(*(uchar_t *)_EWL_LOCALDATA(__lconv).decimal_point);
#endif
  ...
  *overflow = 0;
  c = fetch();
  while ((count <= max_width) && (c != (uint_t)EOF) &&
    ...
    case nan_state: {
      ...
      if ((i == 3) || (i == 4)) {
        if (i == 4) {
          ...
          while ((j < 32) && (iswdigit((wint_t)c) || iswalpha((wint_t)c) ||
                              (c == radix_marker))) {
            nan_arg[j++] = (char_t)c;
            c = fetch();
          }

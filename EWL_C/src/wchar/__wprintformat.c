...
static const wchar_t *parse_format(const wchar_t *format_string, va_list *arg,
                                   print_format *format) {
  print_format f;
  ...
  /* CONVERSION n  argument is (int *) where number of written characters is to
   * be placed */
  case L'n':
    if (f.argument_options == (uchar_t)long_double_argument) {
#if _EWL_LONGLONG
  ...
  *--p = (wchar_t)0;
  ...
  if (minus) {
    ...
    *--p = L'+';
    ...
    if (format.sign_options == (uchar_t)space_holder) {
      *--p = L' ';
    }
  }
...
static wchar_t *justify_conversion(uint_t base, int_t digits, int_t minus,
                                   wchar_t *p, wchar_t *buff,
                                   print_format format) {
  if ((base == 8) && format.alternate_form && (*p != L'0')) {
    *--p = L'0';
    ++digits;
...
static wchar_t *double2hex(ldbl_t num, wchar_t *buff,
  ...
  wchar_t
      *p; /* p points to char array that will be the ascii representation of fp
             # */
  ...
  case '0':
    ...
    if (dec.sgn) {
      ...
      if (format.conversion_char == L'A') {
        MISRA_EXCEPTION_RULE_1_2a()(void) wcscpy(p, L"-0X0");
      } else {
        MISRA_EXCEPTION_RULE_1_2a()(void) wcscpy(p, L"-0x0");
      }
    } else {
  ...
  case 'I':
    ...
    if (dec.sgn) {
      p = buff - 5;
      if (format.conversion_char == L'A') {
        MISRA_EXCEPTION_RULE_1_2a()(void) wcscpy(p, L"-INF");
      } else {
        MISRA_EXCEPTION_RULE_1_2a()(void) wcscpy(p, L"-inf");
      }
    } else {
  ...
  case 'N':
    ...
    if (dec.sgn) {
      p = buff - 5;
      if (format.conversion_char == L'A') {
        MISRA_EXCEPTION_RULE_1_2a()(void) wcscpy(p, L"-NAN");
      } else {
        MISRA_EXCEPTION_RULE_1_2a()(void) wcscpy(p, L"-nan");
      }
    } else {
...
#error fix for greater-than-32-bit exponents
  ...
  for (hex_precision = format.precision; hex_precision >= 1; hex_precision--) {
    if (mantissa_bit < (uint_t)TARGET_FLOAT_BITS) {
      ...
      working_byte =
          ((uint_t)(*(q + mantissa_byte)) >> (7u - (mantissa_bit & 7u)));

      /* nybble crosses a byte */
      if ((mantissa_bit & (uint_t) ~(uint_t)7u) !=
          ((mantissa_bit - 4u) & (uint_t) ~(uint_t)7u)) {
        working_byte |= ((uint_t)(*(q + (mantissa_byte - 1u))) << 8u) >>
                        (7u - ((mantissa_bit)&7u));
      }
...
static wchar_t *float2str(ldbl_t num, wchar_t *wbuff, print_format format) {
  ...
  case 'I':
    if (num < 0.0L) {
      ...
      if (iswupper(format.conversion_char)) {
        MISRA_EXCEPTION_RULE_1_2a()(void) wcscpy(pw, L"-INF");
      } else {
        MISRA_EXCEPTION_RULE_1_2a()(void) wcscpy(pw, L"-inf");
      }
    } else {
  ...
  case 'N':
    if (dec.sgn) {
      pw = wbuff - 5;
      if (iswupper(format.conversion_char)) {
        MISRA_EXCEPTION_RULE_1_2a()(void) wcscpy(pw, L"-NAN");
      } else {
        MISRA_EXCEPTION_RULE_1_2a()(void) wcscpy(pw, L"-nan");
      }
    } else {
...
int_t __wpformatter(void *(_EWL_CDECL *wWriteProc)(void *a, const wchar_t *b,
                                                   size_t c),
                    void *WriteProcArg, const wchar_t *format_str, va_list arg,
                    int_t is_secure);
int_t __wpformatter(void *(_EWL_CDECL *wWriteProc)(void *a, const wchar_t *b,
                                                   size_t c),
                    void *WriteProcArg, const wchar_t *format_str, va_list arg,
  ...
  while (*format_ptr) {
    ...
    if (!curr_format) {
      ...
      MISRA_EXCEPTION_RULE_12_4()
      if (num_chars &&
          !(*wWriteProc)(WriteProcArg, format_ptr, (size_t)num_chars)) {
        MISRA_EXCEPTION_RULE_14_7()
        return (-1);
      }
      break;
    ...
    case L'i':
      ...
      if ((format.argument_options == (uchar_t)long_long_argument) ||
        ...
        buff_ptr =
            longlong2str(long_long_num, buff + conversion_buff_size, format);
        if (!buff_ptr) {
          MISRA_EXCEPTION_RULE_14_4()
    ...
    case L'n':
      ...
      MISRA_EXCEPTION_RULE_1_2c()
          MISRA_QUIET_SUSPICIOUS_PTR_CAST() switch (format.argument_options) {
    ...
    conversion_error:
      ...
      MISRA_EXCEPTION_RULE_12_4()
      if (num_chars &&
          !(*wWriteProc)(WriteProcArg, curr_format, (uint_t)num_chars)) {
        MISRA_EXCEPTION_RULE_14_7()
        return (-1);
      }
    ...
    if (format.justification_options != (uchar_t)left_justification) {
      fill_char =
          (format.justification_options == (uchar_t)zero_fill) ? L'0' : L' ';
    ...
    MISRA_EXCEPTION_RULE_12_4()
    if (num_chars &&
        !(*wWriteProc)(WriteProcArg, buff_ptr, (uint_t)num_chars)) {
      MISRA_EXCEPTION_RULE_14_7()
      return (-1);
    }

...
static uint_t _EWL_CDECL wemit(wchar_t *str, size_t size, size_t *max_size,
...
size_t _EWL_CDECL wcsftime(wchar_t *_EWL_RESTRICT str, size_t max_size,
                           const wchar_t *_EWL_RESTRICT format_str,
  ...
  while (*format_ptr) {
    ...
    case L'a':
      ...
      num_chars =
          wemit(str, strlen(temp_string), &space_remaining, L"%s", temp_string);
      break;
    ...
    case L'U':
      num_chars =
          wemit(str, 2u, &space_remaining, L"%.2d", __ewl_week_num(&tm, 0));
      break;
    ...
    case L'W':
      num_chars =
          wemit(str, 2u, &space_remaining, L"%.2d", __ewl_week_num(&tm, 1));
      break;
    ...
    case L'Z':
      ...
      MISRA_EXCEPTION_RULE_20_12() {
        ...
        if (utctmptr == NULL) {
          ...
          num_chars = wemit(str, 5u, &space_remaining, L"%+03.0f%02.0f",
                            diffhours, diffmins);
        }
        break;
      }
      ...
      /*
      **			case L'Z':
      **				if (*current_locale_ptr->time_cmpt_ptr->TimeZone ==
      *'\0') *				{ *
      *num_chars = 0; *					str[0] = '\0'; *
      *} *				else
      **				    num_chars = wemit(str,
      *strlen(current_locale_ptr->time_cmpt_ptr->TimeZone), &space_remaining,
      *L"%s", *

...
static int_t tst_char_map(const char_map *cm, wchar_t c) {
  ...
  while ((str < cm->stop) && !found) {
    if (*str == c) {
      found = 1;
    } else if ((str[0] < c) && (str[1] == L'-') && (c <= str[2]) &&
               ((str + 2) != cm->stop)) { /* -] to list - in a set */
      found = 1;
    } else if (str[1] == L'-') {
      str += 3;
    } else {
...
static const wchar_t *parse_format(const wchar_t *format_string,
                                   scan_format *format) {
...
int_t __wsformatter(wint_t (*wReadProc)(void *a, wint_t b, int_t c),
  ...
  while (!terminate && ((format_char = *format_ptr) != 0u)) {
    ...
    if ((format_char != L'%') && (!match_failure)) {
      if ((c = (*wReadProc)(wReadProcArg, (uint16_t)0, (int_t)__GetAwChar)) !=
          format_char) {
        (void)(*wReadProc)(wReadProcArg, c, (int_t)__UngetAwChar);
        if (!is_secure) {
    ...
    if ((format.conversion_char != L'n') && (!match_failure) &&
        (*wReadProc)(wReadProcArg, (uint16_t)0, (int_t)__TestForwcsError)) {
      if (!is_secure) {
        MISRA_EXCEPTION_RULE_14_4()
        goto exit;
      } else {
    ...
    signed_int_assign:
      if (arg_ptr) {
        MISRA_EXCEPTION_RULE_1_2c() MISRA_QUIET_SUSPICIOUS_PTR_CAST()
            MISRA_QUIET_ENUM_SWITCH() switch (format.argument_options) {
        ...
        case size_t_argument:
          *(size_t *)arg_ptr = (size_t)long_num;
          break;
    ...
    unsigned_int:
      if (match_failure) {
        ...
        {
          u_long_num =
              __wcstoul(base, format.field_width, wReadProc, wReadProcArg,
                        &num_chars, &negative, &overflow);
        }
      ...
      if (arg_ptr) {
        ...
        case short_argument:
          *(uint16_t *)arg_ptr = (uint16_t)u_long_num;
          break;
        case long_argument:
          *(uint32_t *)arg_ptr = u_long_num;
          break;
#if _EWL_LONGLONG
#if _EWL_C99_PRINTF_SCANF
        case intmax_argument:
          *(intmax_t *)arg_ptr = (intmax_t)u_long_long_num;
          break;
#endif
#else
        case intmax_argument:
        ...
        case size_t_argument:
          *(size_t *)arg_ptr = u_long_num;
          break;
        case ptrdiff_argument:
          *(ptrdiff_t *)arg_ptr = (ptrdiff_t)u_long_num;
          break;
#if _EWL_C99_PRINTF_SCANF
#if _EWL_LONGLONG
        case long_long_argument:
    ...
    case L'G':
      if (match_failure) {
        ...
        long_double_num = __wcstold(format.field_width, wReadProc, wReadProcArg,
                                    &num_chars, &overflow);
        if (!num_chars) {
          if (!is_secure) {
            MISRA_EXCEPTION_RULE_14_4()
    ...
    assign_float:
      if (arg_ptr) {
        ...
        case double_argument:
          *(f64_t *)arg_ptr = (f64_t)long_double_num;
          break;
      ...
      if (arg_ptr) {
        ...
        if (match_failure) {
          ...
          while (format.field_width-- &&
                 (!is_secure ||
                  ((elem_valid = (elem_maxsize > (size_t)num_chars)) != 0)) &&
                 ((c = (*wReadProc)(wReadProcArg, (uint16_t)0,
                                    (int_t)__GetAwChar)) != WEOF)) {
      ...
      if (arg_ptr) {
        ...
        case short_argument:
          *(int16_t *)arg_ptr = (int16_t)chars_read;
          break;
        case long_argument:
          *(int32_t *)arg_ptr = (int32_t)chars_read;
          break;
        case char_argument:
          *(char_t *)arg_ptr = (char_t)chars_read;
          break;
#if _EWL_C99_PRINTF_SCANF
#if _EWL_LONGLONG
        case long_long_argument:
...
exit:
  ...
  if ((*wReadProc)(wReadProcArg, (wint_t)0, (int_t)__TestForwcsError) &&
      (conversions == 0)) {
    MISRA_EXCEPTION_RULE_14_7()
    return (EOF);
  }

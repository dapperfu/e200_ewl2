...
MISRA_QUIET_LINT_04()
size_t _EWL_CDECL strxfrm(char_t *_EWL_RESTRICT str1,
                          const char_t *_EWL_RESTRICT str2, size_t n) {
  ...
  uchar_t *chptr, *collptr, *weight1, *weight2, *localstr1, *wt1ptr, *wt2ptr,
      *tempptr;
  ...
  for (i = 0; i < lenstr2; i++) {
    ...
    if ((chrval >= offset) && (chrval <= (offset + tabsize))) {
      ch = *(current_locale_ptr->coll_cmpt_ptr->char_coll_table_ptr +
             (chrval - offset));
      if (ch != 0u) {

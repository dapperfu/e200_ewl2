 ...
 *	Implementation
 *	--------------
 *		Specialized routines to copy memory by transferring longs and
 *using unrolled loops whenever possible.
 *
 *		There are two pairs of routines.
 *
 *		One pair handles the cases where both the source and destination
 *are mutually aligned, in which case it suffices to transfer bytes until they
 *		are long-aligned, then transfer longs, then finish up transferring
 *any excess bytes.
 *
 *		The other pair handles the cases where the source and destination
 *are not mutually aligned. The handling is similar but requires repeatedly
 *		assembling a destination long from two source longs.
 *
 *		Each pair consists of one routine to transfer bytes from first
 *byte to last and another to transfer bytes from last byte to first. The latter
 *should be used whenever the address of the source is less than the address
 *of the destination in case they overlap each other.
 *
 *		There are also two routines here that effectively duplicate memcpy
 *and memmove, so that these routines may be used without bringing in the entire
 *		StdLib.
 *
 *		Oh, and let's not forget __fill_mem, which fills memory a long at
 *a time and in an unrolled loop whenever possible.
 *
 *
 */
...
void _EWL_CDECL __copy_longs_aligned(void *_EWL_RESTRICT dst, const void *src,
                                     uint32_t n) {
#if (defined(__MOTO__) || defined(__MIPS__) || defined(__GNUC__) ||            \
     defined(_GHSPORT_))

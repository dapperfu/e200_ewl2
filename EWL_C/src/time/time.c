...
const char_t __ewl_wday_name[7][10] = {"Sunday",    "Monday",   "Tuesday",
...
/*
 *	leap_year - return nonzero if year is a leap year, zero otherwise (year
 *0 = 1900)
 */

int_t __ewl_leap_year(int_t year) {
  MISRA_EXCEPTION_RULE_12_4()
  return ((__ewl_mod(year, 4) == 0) &&
          ((__ewl_mod(year, 100) != 0) || (__ewl_mod(year, 400) == 100)));
}
...
/*
 *	__ewl_time2tm - convert seconds since midnight, 1/1/1900 (or 1970 on
 *Win32), to broken-down time
 */
...
void __ewl_time2tm(time_t inTime, struct tm *tm) {
  ...
  */
  ...
  for (;;) {
    uint32_t days_thru_this_month =
        (uint32_t)__ewl_month_to_days[is_leap_year][months + 1u];
...
static int_t __tm2time(struct tm *tm, time_t *tme) {
  ...
  seconds =
      ((uint32_t)tm->tm_hour * seconds_per_hour) + /* convert HMS to seconds */
      ((uint32_t)tm->tm_min * seconds_per_minute) + (uint32_t)tm->tm_sec;
...
/*
 *	__ewl_asciitime - similar to asctime, but requires a pointer to result
 *string as input
 */
...
void __ewl_asciitime(struct tm tm, char_t *str) {
...
#else
  {
    ...
    strcpy(day_name,
           __EWL_extract_name(current_locale_ptr->time_cmpt_ptr->Day_Names,
                              tm.tm_wday * 2));
    strcpy(month_name,
           __EWL_extract_name(current_locale_ptr->time_cmpt_ptr->MonthNames,
                              tm.tm_mon * 2));
  }
#endif

  sprintf(str, "%s %s%3d %.2d:%.2d:%.2d %d\n", day_name, month_name, tm.tm_mday,
          tm.tm_hour, tm.tm_min, tm.tm_sec, tm.tm_year + 1900);
}

/*
 *	__ewl_clear_tm - sets a broken-down time to the equivalent of 1900/1/1
 *00:00:00
 */
...
clock_t _EWL_CDECL clock(void) { return (__get_clock()); }
...
errno_t _EWL_CDECL asctime_s(char_t *s, rsize_t maxsize, const struct tm *tm) {
  ...
  if (mktime(&asc_tm) == (time_t)-1) {
    ...
    n = snprintf(s, maxsize, "%.3s %.3s %2d %.2d:%.2d:%.2d %4d\n", day_name,
                 month_name, asc_tm.tm_mday, asc_tm.tm_hour, asc_tm.tm_min,
                 asc_tm.tm_sec, year);
  }
...
struct tm *_EWL_CDECL gmtime_s(const time_t *_EWL_RESTRICT timer,
                               struct tm *_EWL_RESTRICT tm) {
  time_t tme;
...
struct tm *_EWL_CDECL localtime_s(const time_t *_EWL_RESTRICT timer,
                                  struct tm *_EWL_RESTRICT tm) {
  time_t tme;
...
static size_t emit(char_t *str, size_t size, size_t *max_size,
...
int_t __EWL_ISO8601Week(const struct tm *tmptr, int_t *WYear) {
  ...
  if ((LastYear.Start <= GivenTimeVal) && (GivenTimeVal < ThisYear.Start)) {
    WyPtr = &LastYear;
  } else if ((ThisYear.Start <= GivenTimeVal) &&
             (GivenTimeVal < NextYear.Start)) {
    WyPtr = &ThisYear;
  } else {
    WyPtr = &NextYear;
  }
...
size_t _EWL_CDECL strftime(char_t *_EWL_RESTRICT str, size_t max_size,
  ...
  while (*format_ptr) {
    ...
    case 'a':
      ...
      strcpy(temp_string,
             __EWL_extract_name(
                 _EWL_LOCALDATA(_current_locale).time_cmpt_ptr->Day_Names,
                 tm.tm_wday * 2));
#endif /* _EMBEDDED_WARRIOR_HAS_NO_LOCALE */
      num_chars =
          emit(str, strlen(temp_string), &space_remaining, "%s", temp_string);
      break;
    ...
    case 'h':
      ...
      strcpy(temp_string,
             __EWL_extract_name(
                 _EWL_LOCALDATA(_current_locale).time_cmpt_ptr->MonthNames,
                 tm.tm_mon * 2));
#endif /* _EMBEDDED_WARRIOR_HAS_NO_LOCALE */
      num_chars =
          emit(str, strlen(temp_string), &space_remaining, "%s", temp_string);
...
#else
      num_chars = strftime(
          str, space_remaining + 1u,
          _EWL_LOCALDATA(_current_locale).time_cmpt_ptr->DateTime_Format, &tm);
#endif /* _EMBEDDED_WARRIOR_HAS_NO_LOCALE */
      space_remaining -= num_chars;
      break;
...
#if _EWL_C99
    case 'F':
      num_chars = emit(str, 10u, &space_remaining, "%.4d-%.2d-%.2d",
    ...
    case 'I':
      num_chars = emit(str, 2u, &space_remaining, "%.2d",
    ...
    case 'p':
      ...
      name = __EWL_extract_name(
          _EWL_LOCALDATA(_current_locale).time_cmpt_ptr->am_pm, temp);
#endif /* _EMBEDDED_WARRIOR_HAS_NO_LOCALE */
    ...
    case 'U':
      num_chars =
          emit(str, 2u, &space_remaining, "%.2d", __ewl_week_num(&tm, 0));
      break;
    ...
    case 'W':
      num_chars =
          emit(str, 2u, &space_remaining, "%.2d", __ewl_week_num(&tm, 1));
      break;
    ...
    case 'x':
      ...
      num_chars = strftime(
          str, space_remaining + 1u,
          _EWL_LOCALDATA(_current_locale).time_cmpt_ptr->Date_Format, &tm);
#endif /* _EMBEDDED_WARRIOR_HAS_NO_LOCALE */
      space_remaining -= num_chars;
      break;
    ...
    case 'X':
      ...
      num_chars = strftime(
          str, space_remaining + 1u,
          _EWL_LOCALDATA(_current_locale).time_cmpt_ptr->Time_Format, &tm);
#endif /* _EMBEDDED_WARRIOR_HAS_NO_LOCALE */
      space_remaining -= num_chars;
      break;
    ...
    case 'Y':
      num_chars = emit(str, 4u, &space_remaining, "%.4d",
    ...
    case 'z': {
      ...
      if (utctmptr == NULL) {
        ...
        num_chars = emit(str, 5u, &space_remaining, "%+03.0f%02.0f", diffhours,
                         diffmins);
      }
      break;
    }
    ...
    case 'Z':
      ...
      if (*_EWL_LOCALDATA(_current_locale).time_cmpt_ptr->TimeZone == '\0') {
        ...
        num_chars = emit(
            str,
            strlen(_EWL_LOCALDATA(_current_locale).time_cmpt_ptr->TimeZone),
            &space_remaining, "%s",

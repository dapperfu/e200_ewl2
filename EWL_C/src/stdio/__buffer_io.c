...
int_t _EWL_CDECL __load_buffer(FILE *_file, size_t *bytes_loaded,
                               int_t alignment) {
  MISRA_EXCEPTION_RULE_11_4()
  struct _FILE *file = (struct _FILE *)_file;
  ...
  ioresult = (__io_results)(*file->read_proc)(
      file->handle, file->buffer, (size_t *)&file->buffer_len, file->ref_con);
#else
  ioresult = (__io_results)(*file->read_proc)(file->handle, file->buffer,

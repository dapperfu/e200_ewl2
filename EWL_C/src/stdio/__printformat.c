...
#warning                                                                       \
    "Impossible to perform long long IO when long long support is not available!"
#endif
...
#define vec_va_arg(ap)                                                         \
  (*(((MWVector128 *)(ap = (void *)((((uint32_t)ap + 15) & ~(15)) + 16))) - 1))
...
typedef struct {
  ...
  int_t
      useVecDefault; /* default to "appropriate" vec access if none specified */
...
/* Parse the format specifiers so appropriate types, precision, and formatting
 * are used */
MISRA_EXCEPTION_RULE_16_7()
static const char_t *parse_format(const char_t *format_string, va_list *arg,
                                  print_format *format) {
  print_format f;
...
getFlags:
  ...
  for (;;) {
...
#ifdef __ALTIVEC__
    /* PREFIX @  AltiVec extension, next argument provides vector separator
     * string */
    case '@':
    ...
    /* Any other prefix character is not a flag, so continue processing format
     * arguments */
    default:
      flag_found = 0;
      break;
...
#ifdef __ALTIVEC__
  if (f.useSepString) {
    /* PREFIX @ was used, so obtain the next argument to find the separator
     * string */
  ...
  if (c == '.') {
    ...
    if (c == '*') {
      ...
      /* Precision or string length specified by a decimal number, parse the
       * number */
      while (isdigit(c)) {
...
#ifdef __ALTIVEC__
    /* For AltiVec, precision is 8 with non-vector arguments, 1 with vector
     * arguments */
    if (f.argument_options != vector_argument) {
      f.argument_options = long_argument;
      f.precision = 8;
  ...
  /* CONVERSION n  argument is (int_t *) where number of written characters is
   * to be placed */
  case 'n':
#if _EWL_FLOATING_POINT_PRINTF_SCANF
    if (f.argument_options == long_double_argument) {
  ...
  if (!num && !format.precision &&
    ...
    if ((format.conversion_char == 'd') || (format.conversion_char == 'i')) {
      if (format.sign_options == space_holder) {
         ...
         * if a signed conversion results in no characters, a space is prefixed
         * to the result.
         */
        *--p = ' ';
      } else {
...
#endif /* _EWL_LONGLONG && (_EWL_C99_PRINTF_SCANF ||                           \
          _EWL_LONGLONG_PRINTF_SCANF) */
...
static char_t *justify_conversion(uint_t base, int_t digits, int_t minus,
                                  char_t *p, char_t *buff,
                                  print_format format) {
...
#if _EWL_C99_PRINTF_SCANF && _EWL_FLOATING_POINT_PRINTF_SCANF
static char_t *double2hex(ldbl_t num, char_t *buff,
                          print_format format) { /* num = fp#, buff = ascii
                                                    representation of fp#,
                                                    format = formating of the
                                                    number, i.e. %a */

  char_t *p;  /* p points to char_t array that will be the ascii representation
                 of fp # */
  uchar_t *q; /* q points to fp# */
  uchar_t
      working_byte; /* temporary space to manipulate the fp# as a character */
  ldbl_t ld; /* ld is an internal variable, set equal to the floating point
                number */
  int_t expbits, expmask; /* facts about the exponent representation */
  uint_t
      snum; /* snum is an intermediate value used to calculate the exponent */
  int32_t expo; /* is the exponent of the fp # */
  print_format exp_format;
  int_t hex_precision; /* hex_precision is used as a limit for the loop that
                          writes the ascii fp# */
...
#error fix for greater-than-32-bit exponents
  ...
  if (snum != 0U) {
    /* bias back into real exponent (LDBL_MAX_EXP = 0, LDBL_MAX_EXP+1 = 2^0,
     * LDBL_MAX_EXP-1 = 2^-1) */
  ...
  had_non_zero =
      format.precision_specified; /* emit trailing zeros if precision was
                                     specified */
  for (hex_precision = format.precision; hex_precision >= 1; hex_precision--) {
    if (mantissa_bit < (uint_t)TARGET_FLOAT_BITS) {
      ...
      if ((mantissa_bit & ~(uint_t)7u) != ((mantissa_bit - 4u) & ~(uint_t)7u)) {
        working_byte |=
            (uchar_t)(((uint_t)(*(q + (mantissa_byte - 1u))) << 8u) >>
                      (7u - (mantissa_bit & 7u)));
      }
  ...
  if (TARGET_FLOAT_IMPLICIT_J_BIT) {
    if (format.precision ||
        format.alternate_form) { /* STEP#6: emit implicit "1." */
      *--p = (char_t)radix_marker;
    }
...
/* Exposes double2hex to the outside. Currently used by the C++ lib num_put
 * facet */
/* because it is far more efficient than using the public interface (sprintf).
 */
char_t *__double2hex(ldbl_t num, char_t *buff, char_t uppercase,
                     int_t precision) {
  print_format format;
...
static char_t *float2str(ldbl_t num, char_t *buff, print_format format) {
   ...
   * Ergo, the difference is moot *until* a truly long double
   * type is supported.
   */
  ...
  *--p = 0;
  ...
  f_format:
    ...
    if (frac_digits > format.precision) {
      __round_decimal(&dec,
                      (int_t)dec.sig.length - (frac_digits - format.precision));
    ...
    for (digits = 0; (digits < frac_digits) && (digits < (int_t)dec.sig.length);
         ++digits) {
      *--p = *--q;
    }
    ...
    if (int_digits) {
      ...
      for (digits = 0; digits < (int_digits - (int_t)dec.sig.length);
           ++digits) {
        *--p = '0';
      }
#else
...
int_t __pformatter(void *(_EWL_CDECL *WriteProc)(void *a, const char_t *b,
                                                 size_t c),
                   void *WriteProcArg, const char_t *_EWL_RESTRICT format_str,
                   va_list arg, int_t is_secure) {
...
#pragma unused(is_secure)
  ...
  while (*format_ptr) {
    if ((curr_format = strchr(format_ptr, '%')) == 0) {
      ...
      MISRA_EXCEPTION_RULE_12_4()
      if (num_chars &&
          !(*WriteProc)(WriteProcArg, format_ptr, (uint_t)num_chars)) {
        MISRA_EXCEPTION_RULE_14_7()
        return (-1);
      }
      break;
    ...
    MISRA_EXCEPTION_RULE_1_2c() format_ptr =
        parse_format(format_ptr, (va_list *)arg, &format);
#else
    ...
    do { /* can be done repeatedly for each vector field */
      ...
      case 'i':
        ...
        if ((format.argument_options == long_long_argument)
          ...
          if ((buff_ptr = longlong2str(
                   long_long_num, buff + conversion_buff_size, format)) == 0) {
            MISRA_EXCEPTION_RULE_14_4()
      ...
      case 'G':
        ...
        if ((buff_ptr = float2str(long_double_num, buff + conversion_buff_size,
                                  format)) == 0) {
          MISRA_EXCEPTION_RULE_14_4()
          goto conversion_error;
        }
      ...
      case 'A':
        ...
        if ((buff_ptr = double2hex(long_double_num, buff + conversion_buff_size,
                                   format)) == 0) {
          MISRA_EXCEPTION_RULE_14_4()
          goto conversion_error;
        }
      ...
      case 's':
        ...
        if (format.alternate_form) {
          ...
          num_chars = format.precision;
          if ((string_end = (char_t *)memchr(buff_ptr, 0, (uint_t)num_chars)) !=
              0) {
            MISRA_EXCEPTION_RULE_17_3()
      ...
      buff_ptr = va_arg(arg, char_t *);
        ...
        case normal_argument:
          *(int_t *)buff_ptr = chars_written;
          break;
        case short_argument:
          *(int16_t *)buff_ptr = (int16_t)chars_written;
          break;
        case long_argument:
          *(int32_t *)buff_ptr = chars_written;
          break;
        ...
        case size_t_argument:
          *(size_t *)buff_ptr = (size_t)chars_written;
          break;
        ...
        case long_long_argument:
          *(int64_t *)buff_ptr = chars_written;
          break;
#endif /* _EWL_LONGLONG_PRINTF_SCANF */
        default:
          break;
      ...
      case 'r':
        ...
        if (format.argument_options == short_argument) {
          ...
          long_double_num =
              va_arg(arg, long long) / (long double)0x8000000000000000ULL;
        }
      ...
      case 'R':
        ...
        if (format.argument_options == short_argument) {
          long_double_num =
              (unsigned short)va_arg(arg, uint32_t) / (long double)0x10000;
        } else if (format.argument_options == normal_argument) {
          long_double_num = va_arg(arg, uint32_t) / (long double)0x100000000ULL;
        } else {
          long_double_num = va_arg(arg, uint64_t) /
                            (2.0 * (long double)0x8000000000000000ULL);
        }
      ...
      conversion_error:
        ...
        MISRA_EXCEPTION_RULE_12_4()
        if (num_chars &&
            !(*WriteProc)(WriteProcArg, curr_format, (uint_t)num_chars)) {
          MISRA_EXCEPTION_RULE_14_7()
          return (-1);
        }
      ...
      MISRA_EXCEPTION_RULE_12_4()
      if (num_chars &&
          !(*WriteProc)(WriteProcArg, buff_ptr, (uint_t)num_chars)) {
        MISRA_EXCEPTION_RULE_14_7()
        return (-1);
      }
        ...
        if (format.vecSeperator && format.vecSepLen > 0) {
          if (format.vecSepLen == 1) {
            if ((*WriteProc)(WriteProcArg, format.vecSeperator, 1) == 0)
              MISRA_EXCEPTION_RULE_14_7()
            return (-1);
          } else if (!(*WriteProc)(WriteProcArg, format.vecSeperator,
                                   format.vecSepLen))
            MISRA_EXCEPTION_RULE_14_7()
          return (-1);
    ...
    } while (format.argument_options == vector_argument &&
             vecIndex < format.vecCount);
#endif /* __ALTIVEC__ */
  }

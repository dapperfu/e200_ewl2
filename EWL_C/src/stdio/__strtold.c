 ...
 *	--------------
 *
 *		The string scanner is implemented as an extended Finite State
 *Machine. A state diagram for it can be found in an accompanying TeachText
 *		document, 'strtod syntax' (too bad pictures can't be imbedded in
 *		comments) in the "EWL Technical Notes" directory. A textual
 *description of it follows.
 *
 *		The state transition loop dispatches to the appropriate code for
 *the current state, while simultaneously watching for terminating conditions
 *(field width exhausted, EOF encountered, final state reached).
 *
 *		start
 *
 *			Skip leading spaces. Once a non-space is seen, process sign
 *(if any) and trasition to next state.
 *
 *		sig_start
 *
 *			Look for either a digit or a decimal point. If it is a digit
 *zero, treat it specially.
 *
 *		leading_sig_zeroes
 *
 *			Leading zero digits are discarded, as they add nothing to
 *the result.
 *
 *		int_digit_loop
 *
 *			Process digits from the integer part of the significand. We
 *accept only so many significant digits (DBL_DIG), but the ones we discard have
 *to be accounted for in the exponent.
 *
 *			If a decimal point is seen, proceed to process a fractional
 *part (if one is present).
 *
 *		frac_start
 *
 *			Having seen a leading decimal point, we must see at least
 *one digit. If the field width expires before the transition from this state to
 *			the next, we fail.
 *
 *		frac_digit_loop
 *
 *			Process digits from the fractional part of the significand.
 *We accept only so many significant digits (DBL_DIG), but the ones we discard
 *			have to be accounted for in the exponent.
 *		sig_end
 *
 *			If an 'E' (or 'e') follows we go after an exponent;
 ...
 *		leading_exp_digit
 *
 *			Check the leading exponent digit. If it is a digit zero,
 *treat it specially.
 *
 *		leading_exp_zeroes
 *
 ...
 *		exp_digit_loop
 *
 *			Process digits from the exponent. We watch for short int_t
 *overflow, even though the maximum exponent is probably considerably less than
 *			this. The latter will be checked during the actual decimal
 ...
 *		'decimal' record ˆ la SANE and MathLib. '__dec2num' is used for
 *conversion to binary. For other systems that don't provide decimal to binary
...
/*
        ...
        where the real_mantissa is "1.partial_mantissa" for 32 and 64 bit
   doubles and ".partial_mantissa" for 80-bit doubles (with an explicit 1 bit),
...
#define final_state(scan_state)                                                \
  ((uint_t)scan_state & ((uint_t)finished | (uint_t)failure))

#define success(scan_state)                                                    \
  ((uint_t)scan_state &                                                        \
   ((uint_t)leading_sig_zeroes | (uint_t)int_digit_loop |                      \
    (uint_t)frac_digit_loop | (uint_t)leading_exp_zeroes |                     \
    (uint_t)exp_digit_loop | (uint_t)finished))

/* for hex, we can't accept "0x" */
#define hex_success(count, scan_state)                                         \
  (((count - 1) > 2) &&                                                        \
   (scan_state &                                                               \
    ((uint_t)hex_leading_sig_zeroes | (uint_t)hex_int_digit_loop |             \
     (uint_t)hex_frac_digit_loop | (uint_t)hex_leading_exp_zeroes |            \
     (uint_t)hex_exp_digit_loop)))

/*lint -emacro( (960), fetch) MISRA EXPCEPTION RULE 12.10 use of the comma
 * operator */
...
ldbl_t _EWL_CDECL __strtold(
    int_t max_width, int_t(_EWL_CDECL *ReadProc)(void *a, int_t b, int_t c),
    void *ReadProcArg, int_t *chars_scanned, int_t *overflow) {
  ...
  *overflow = 0;
  ...
  while ((count <= max_width) && (c != (int_t)EOF) &&
    ...
    case exp_digit_loop:
      ...
      if (exp_value > (int32_t)SHRT_MAX) { /* overflow check is done below */
  ...
  if (hex_scan_state == not_hex) {
    ...
    if ((exp_value < (int32_t)TARGET_FLOAT_LDBL_MIN_10_EXP) ||
        (exp_value > (int32_t)TARGET_FLOAT_LDBL_MAX_10_EXP)) {
    ...
    /*
     *	Note:	__dec2num() can call library functions like ldexp() that may set
     *errno. so we save and restore the value.
     */
     ...
     *double. If you look at <float.h> you'll see that long double == double.
     *Ergo, the difference is moot *until* a truly long double type is
     *supported.
     */
    ...
    */
    if (result == 0.0L) {
      if (d.sig.text[0] !=
          '0') { /*	a zero result with a non-zero mantissa, this is an
                    underflow */
        *overflow = 1;
      }
    } else {
    ...
    for (ui = 0;; ++ui) {
      if (ui >=
          TARGET_FLOAT_BYTES) { /* mantissa is zero, so we just return 0.0 */
        MISRA_EXCEPTION_RULE_14_7()
        return 0.0;
      }
      if (mantissa[ui] != 0u) {
    ...
    */
    ...
    dbl_bit =
        (uint_t)(TARGET_FLOAT_BITS -
                 TARGET_FLOAT_MANT_BITS); /* point to mantissa inside double */

    /* pack eight bits at a time */
    for (mantissa_bit = 0U; mantissa_bit < (uint_t)TARGET_FLOAT_MANT_BITS;
         mantissa_bit += 8U) {
      ...
      if ((mantissa_bit + 8U) > (uint_t)TARGET_FLOAT_MANT_BITS) {
        lui &=
            (uchar_t)(0xffu << ((uint_t)TARGET_FLOAT_MANT_BITS - mantissa_bit));
      }
    ...
    /* exponent is biased by LDBL_MAX_EXP so p0 = LDBL_MAX_EXP, p1=...+1,
     * p-1=...-1 */
    MISRA_EXCEPTION_RULE_10_3()
...
ldbl_t _EWL_CDECL strtold(const char_t *_EWL_RESTRICT nptr,
                          char_t **_EWL_RESTRICT endptr) {
  ldbl_t value;
...
f64_t _EWL_CDECL strtod(const char_t *_EWL_RESTRICT str,
                        char_t **_EWL_RESTRICT end) {
  f64_t value;
  int_t count, overflow;
  ...
  MISRA_EXCEPTION_RULE_11_4()
  value =
      (f64_t)__strtold(INT_MAX, __StringRead, (void *)&isc, &count, &overflow);
...
f32_t _EWL_CDECL strtof(const char_t *_EWL_RESTRICT str,
                        char_t **_EWL_RESTRICT end) {
  f32_t value;
  int_t count, overflow;
  ...
  MISRA_EXCEPTION_RULE_11_4()
  value =
      (f32_t)__strtold(INT_MAX, __StringRead, (void *)&isc, &count, &overflow);
...
f64_t _EWL_CDECL atof(const char_t *str) { return (strtod(str, NULL)); }

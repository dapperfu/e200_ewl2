...
#if !_EWL_OS_DISK_FILE_SUPPORT &&                                              \
    (!_EWL_CONSOLE_SUPPORT || !_EWL_BUFFERED_CONSOLE)
#define _EWL_BUFFERED_IO 0
#else
#define _EWL_BUFFERED_IO 1
#endif /* !_EWL_OS_DISK_FILE_SUPPORT && (!_EWL_CONSOLE_SUPPORT) ||             \
          !_EWL_BUFFERED_CONSOLE) */
#else
...
/* This does all the work of fwrite but is not threadsafe it exists so that
   other library functions can do fwrites in a loop from within a critical
   region mm 001018*/
size_t _EWL_CDECL __fwrite(const void *ptr, size_t memb_size, size_t num_memb,
                           FILE *_file) {
  ...
  if (!bytes_to_go || file->state.error ||
...
size_t _EWL_CDECL fwrite(const void *_EWL_RESTRICT ptr, size_t memb_size,
                         size_t num_memb, FILE *_EWL_RESTRICT file) {
  size_t retval;
...
void *_EWL_CDECL __FileWrite(void *File, const char_t *Buffer,
                             size_t NumChars) {
  return ((__fwrite(Buffer, 1U, NumChars, (FILE *)File) == NumChars)
              ? File
              : 0U); /* std IO call */
}

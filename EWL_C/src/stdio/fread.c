...
size_t _EWL_CDECL __fread(void *ptr, size_t memb_size, size_t num_memb,
                          FILE *_file) {
  ...
  always_buffer =
      !file->mode.binary_io || (file->mode.buffer_mode == (uchar_t)_IOFBF);
  ...
  if (file->state.io_state >= (uchar_t)__rereading) {
    do {
      ...
      if (fwide(_file, 0) == 1) {
        ...
        *(wchar_t *)read_ptr =
            file->ungetwc_buffer[file->state.io_state - (uchar_t)__rereading];
        read_ptr += sizeof(wchar_t);
        bytes_read += sizeof(wchar_t);
        bytes_to_go -= sizeof(wchar_t);
      } else {
        *read_ptr++ =
            file->ungetc_buffer[file->state.io_state - (uchar_t)__rereading];
        bytes_read++;
        bytes_to_go--;
      }
#else
...
size_t _EWL_CDECL fread(void *_EWL_RESTRICT ptr, size_t memb_size,

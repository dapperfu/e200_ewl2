...
#if _EWL_WIDE_CHAR
    {(uchar_t)__create_or_truncate, (uchar_t)__read_write_mode, (uchar_t)_IOFBF,
     (uchar_t)__disk_file,          (uchar_t)__unoriented,      1u};
#else
    {(uchar_t)__create_or_truncate, (uchar_t)__read_write_mode, (uchar_t)_IOFBF,
     (uchar_t)__disk_file, 1u};
...
FILE *_EWL_CDECL fopen(const char_t *_EWL_RESTRICT name,
                       const char_t *_EWL_RESTRICT mode) {
  FILE *file;
...
#if defined(__STDC_WANT_LIB_EXT1__) && __STDC_WANT_LIB_EXT1__
errno_t _EWL_CDECL fopen_s(FILE *_EWL_RESTRICT *_EWL_RESTRICT file,
                           const char_t *_EWL_RESTRICT name,
                           const char_t *_EWL_RESTRICT mode) {
  FILE *newfile;
...
FILE *_EWL_CDECL freopen(const char_t *_EWL_RESTRICT name,
                         const char_t *_EWL_RESTRICT mode,
                         FILE *_EWL_RESTRICT file) {
  __file_modes modes;
...
#if defined(__STDC_WANT_LIB_EXT1__) && __STDC_WANT_LIB_EXT1__
errno_t _EWL_CDECL freopen_s(FILE *_EWL_RESTRICT *_EWL_RESTRICT newfile,
                             const char_t *_EWL_RESTRICT name,
                             const char_t *_EWL_RESTRICT mode,
                             FILE *_EWL_RESTRICT file) {
  __file_modes modes;
...
/* The function __reopen(FILE* file) is a non-standard function that will change
   the mode of one of the three standard files, stdin, stdout, stderr to binary
   mode from text mode.  The value of the parameter file may only be one of
   stdin, stdout or stderr otherwise the function will take no action and return
   the value NULL.
*/
...
FILE *_EWL_CDECL __handle_reopen(__file_handle handle, const char_t *mode,
                                 FILE *file) {
  __file_modes modes;

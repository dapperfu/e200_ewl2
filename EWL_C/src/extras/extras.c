...
char *_EWL_CDECL ultoa(unsigned long val, char *str,
                       int radix) _EWL_CANT_THROW {
...
int _EWL_CDECL strnicmp(const char *s1, const char *s2,
                        size_t n) _EWL_CANT_THROW {
...
int _EWL_CDECL strncasecmp(const char *s1, const char *s2,
                           size_t n) _EWL_CANT_THROW {
  return __ewl_strnicmp(s1, s2, n);
}
#endif /* _EWL_C_TINY_IO */
...
#if !_EWL_C_TINY_IO
int _EWL_CDECL strncmpi(const char *s1, const char *s2,
                        size_t n) _EWL_CANT_THROW {
  return __ewl_strnicmp(s1, s2, n);
}
#endif /* _EWL_C_TINY_IO */
...
int _EWL_CDECL strncoll(const char *s1, const char *s2,
                        __std(size_t) sz) _EWL_CANT_THROW {
  return strncmp(s1, s2, sz);
}

int _EWL_CDECL strnicoll(const char *s1, const char *s2,
                         __std(size_t) sz) _EWL_CANT_THROW {
...
char *_EWL_CDECL strrev(char *str) _EWL_CANT_THROW { return __ewl_strrev(str); }
#endif /* _EWL_C_TINY_IO*/
...
int _EWL_CDECL chsize(int handle, long size) _EWL_CANT_THROW {
  ...
  if ((fstat(handle, &buff)) == 0) {
    ...
    /* fstat not able to obtain the file-status information -- errno set by
     * fstat */
    return -1;
  }
...
int _EWL_CDECL wcsnicmp(const wchar_t *s1, const wchar_t *s2,
                        size_t n) _EWL_CANT_THROW {
  int i;
  wchar_t c1, c2;
  for (i = 0; i < n; i++) {
...
wchar_t *_EWL_CDECL wcsnset(wchar_t *str, wchar_t wc,
                            size_t n) _EWL_CANT_THROW {
  wchar_t *saved = str;
...
/* return pointer to first character in s1 that isn't in s2 */
wchar_t *_EWL_CDECL wcsspnp(const wchar_t *s1,
                            const wchar_t *s2) _EWL_CANT_THROW {
  int len;
  len = wcsspn(s1, s2);
  if (len < wcslen(s1))
...
int _EWL_CDECL wcsncoll(const wchar_t *s1, const wchar_t *s2,
                        __std(size_t) sz) _EWL_CANT_THROW {
  return wcsncmp(s1, s2, sz);
}

int _EWL_CDECL wcsnicoll(const wchar_t *s1, const wchar_t *s2,
                         __std(size_t) sz) _EWL_CANT_THROW {
...
struct tm *_EWL_CDECL localtime_r(const time_t *timer,
                                  struct tm *tm) _EWL_CANT_THROW {
  return localtime_s(timer, tm);
}

struct tm *_EWL_CDECL gmtime_r(const time_t *timer,
                               struct tm *tm) _EWL_CANT_THROW {
...
int _EWL_CDECL strerror_r(int errnum, char *str,
                          size_t buflen) _EWL_CANT_THROW {
...
char *_EWL_CDECL strtok_r(char *_EWL_RESTRICT str,
                          const char *_EWL_RESTRICT set,
                          char **_EWL_RESTRICT ptr) {

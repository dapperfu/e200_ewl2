...
static int _InitializeThreadDataIndex(void) {
  ...
  /* Call free() on the thread data when a thread terminates and it has local
...
/* This function sets up a structure of thread local data needed by various EWL
 * functions. */
/* Returns false on error, true on success. */
static int _InitializeThreadData(void) {
  ...
  if (tld == NULL) {
    ...
    /* All initialization of items in the _ThreadLocalData structure goes
     * here... */
    ...
    memcpy((void *)&tld->_current_locale.next_locale,
           (void *)&_current_locale.next_locale, sizeof(struct __locale));

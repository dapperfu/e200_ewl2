...
#define CHECK_MUL_OVERFLOW(n, s)                                               \
  if ((((n) > MAX_OV) || ((s) > MAX_OV)) && ((((size_t)-1) / (n)) < (s))) {    \
    return NULL;                                                               \
  }
_MISRA_RESTORE()
...
struct m_buff {
  ...
  struct m_buff
      *lnk; /* link to next contiguous buffer. this member could
...
#pragma noinline
  asm_header return in $r0;
  asm_body;
  PRAGMA stack_effect _get_stack_pointer, 0 tfra sp, r0 nop nop asm_end
}
#elif defined(__COLDFIRE__) && __COLDFIRE__
static char_t *get_stack_pointer(void) {
#ifndef _lint
...
static __asm__ char_t *get_stack_pointer(void) {
#pragma NO_ENTRY
#pragma NO_EXIT
  LEA X, (2, S)RTS
}
...
extern uint_t _LNK_BottomOfHeap_c0, _LNK_BottomOfHeap_c1, _LNK_BottomOfHeap_c2,
    _LNK_BottomOfHeap_c3, _LNK_BottomOfHeap_c4, _LNK_BottomOfHeap_c5;

uint_t *__BottomOfHeap[] = {&_LNK_BottomOfHeap_c0, &_LNK_BottomOfHeap_c1,
                            &_LNK_BottomOfHeap_c2, &_LNK_BottomOfHeap_c3,
                            &_LNK_BottomOfHeap_c4, &_LNK_BottomOfHeap_c5};
...
int_t brk(void *addr) {
  char_t *compare = (char_t *)addr;
#if defined(_MSC8156_)
  char_t *mem_limit_end =
      (char_t *)(((uint_t)get_bottomOfHeap() + 7u) & 0xfffffff8u);
#else
  char_t *mem_limit_end =
      (char_t *)(((uint_t)(&_BottomOfHeap) + 7u) & 0xfffffff8u);
#endif
  if (__size == mem_limit_end) {
    /* it is a dynamic configuration for stack and heap
     ** the dynamic configuration means that the stack and heap are shared the
     *same memory space.
     */
    char_t *stp;
    ...
    if (!((uint32_t)readSR() & ((uint32_t)0x1u << 18))) {
      stp =
          (char_t *)readOSP(); /* The ESP register is read by using the Other
                                  Stack Pointer*/
      mem_limit_end =
          (char_t *)((unsigned long)stp + (unsigned long)__stack_safety);
    } else {
...
void *malloc(size_t nbytes) {
  ...
  /* if the next multiple of 8 number cannot be represented on size_t, return
   * NULL */
  ...
  /* First: attempt to allocate the buffer from the existing pool. Use first-fit
   * algorithm. */
  ...
  if (NULL != first_fit) {
    if (waste < MIN_WASTE_FOR_SPLIT) {
...
#if HEAP_GROWS
      MISRA_EXCEPTION_RULE_11_3()
      new_buff = (struct m_buff * __EWL_ALIGNPTR)((char_t *)first_fit + nbytes +
                                                  sizeof(struct m_buff));
      ...
      MISRA_EXCEPTION_RULE_11_3()
      first_fit =
          (struct m_buff * __EWL_ALIGNPTR)(((char_t *)first_fit) + waste);
      LOCALALLOC(first_fit, nbytes)
  ...
  while (NULL != p) {
    if (NO == p->used) {
      ...
      while ((NULL != look_ahd) && (NO == look_ahd->used) &&
             (tot_room < nbytes)) {
  ...
  MISRA_EXCEPTION_RULE_11_3()
  new_buff = (struct m_buff * __EWL_ALIGNPTR)
      sbrk((uint32_t)nbytes + sizeof(struct m_buff));
...
MISRA_EXCEPTION_RULE_14_7()
void *realloc(void *oldp, size_t nbytes) {
  if (0U == nbytes) {
    return NULL;
  }
  ...
  {
    MISRA_EXCEPTION_RULE_11_3()
    struct m_buff *old_buff =
        (struct m_buff *)((uint32_t)oldp - sizeof(struct m_buff));
    char_t *newp;
...
void _EWL_CDECL free(void *ptr) {
  if (ptr != 0)
    __sys_free(ptr);
}

void *_EWL_CDECL realloc(void *ptr, size_t size) {
  size_t orig_size;
  void *p;
  if (ptr == 0)
    return malloc(size);
...
void *_EWL_CDECL calloc(size_t nmemb, size_t size) {
  void *result;
  CHECK_MUL_OVERFLOW(nmemb, size);
  size *= nmemb;
  result = malloc(size);
...
void _EWL_CDECL __pool_free_all(__mem_pool *x) {
#pragma unused(x)
}

void _EWL_CDECL __malloc_free_all(void) {}
...
int _EWL_CDECL _EWL_ALLOCATE_RESIZE(void *ptr, size_t size_requested,
...
int _EWL_CDECL _EWL_ALLOCATE_EXPAND(void *ptr, size_t min_size,
 ...
 *	Routines
 *	--------
 *		init_alloc			Initialize allocs' memory pool to be
 ...
 *		realloc
 *		free				Return memory block to allocs' memory
 *pool
 */
...
/***************************************************************************
 *	init_alloc
 *		Initializes the memory pool that malloc uses. This call is
 *designed to be used for systems that DO NOT have memory allocation functions
 *  	built into the system. The first call will define the pool and use the
 *		rest of the space as a block of memory. Subsequent calls will add
 *the memory as another block of memory to the malloc's existing pool. Once a
 *heap of memory is allocated to the malloc memory pool, it can not be
 *returned.
 *
 *	Inputs:
 *		pool_ptr		pointer to memory to be used in malloc's
 *memory pool
 *		size				size of memory to be used in (or added
 *to) malloc's memory pool
 *
 *	Outputs:
 *		None
 *
 *	Returns: int
 *		0		Fail
 *		1		Success: Memory initialized as malloc's memory
 *pool 2		Success: Memory added to initial heap successfully
 *
 *  NOTE:
 *		This function must be called prior to using other alloc.c
 *functions
 *
 ***************************************************************************/
extern int _EWL_CDECL init_alloc(void *pool_ptr, mem_size size) {
  if (!initialized) {
    ...
    __pool_preassign(__malloc_pool_ptr,
                     (void *)((size_t)pool_ptr + pool_overhead), size);
...
static const size_t allocate_mask =
    _ALLOC_ALIGNMENT == 1 ? (size_t)(-1) << (CHAR_BIT * sizeof(size_t) - 1) : 1;

#define align_up(size)                                                         \
  ((__typeof__(size))((size_t)size + (_ALLOC_ALIGNMENT - 1) &                  \
                      ~(_ALLOC_ALIGNMENT - 1)))
...
_EWL_MALLOC_TEST uchar_t **insert(uchar_t **e, uchar_t *value, uchar_t **p,
                                  size_t *nblocks);
...
_EWL_MALLOC_TEST uchar_t **lower_bound(uchar_t **first, uchar_t **last,
                                       uchar_t *value);

/*
        static void init_default_heap(void)

        This private function is called to initialize the heap, on the first
   call to __allocate.  It does a run-time check of a few underlying
   assumptions:
        ...
        upwards, with the 0th element at the highest address in the array.  The
   first pointer is set to the beginning of the heap, and implicitly marked
   free. The second pointer points to itself, or the next lowest address which
   is a multiple of _ALLOC_ALIGNMENT.  This second pointer is marked allocated.
        ...
        If _EWL_HEAP_SIZE falls below a size which it would be impossible to set
   up the heap overhead and still have a free block, init_default_heap will do
   nothing. Clients must check for this condition by inspecting
   defaultheapinitialized after the call.
*/
...
void init_default_heap(void) {
  if (_EWL_HEAP_SIZE >=
    ...
    if (_ALLOC_ALIGNMENT == 1)
      assert((((size_t)__heap + _EWL_HEAP_SIZE) &
              ((size_t)(-1) << (CHAR_BIT * sizeof(size_t) - 1))) == 0);
    *array_size() = 2;
    *array(0) = __heap;
    p = array(1);
...
/*
        unsigned char** insert(unsigned char** e, unsigned char* value, unsigned
   char** p, size_t* nblocks)

        This private function attempts to insert a new pointer just after p (at
   p-1), And give it the value value.  It moves all pointers after p down by
   one.  The pointer e marks the end of the array.  nblocks points to the size
   of the array which will be incremented if necessary.

        If the insert is successful, a pointer to the newly inserted pointer is
   returned, else null is returned.  To be successful, there must be room for
   the array to grow by one.  For this to be true, either the end of the array
   currently points to a lower address than itself (perhaps for alignment
   reasons), or the final block in the array has not been allocated, or a new
   free block is being inserted at the end of the array, and thus the array can
   grow into it.

        If the growing array grows into the last free block, it may either split
   the last free block, taking only _ALLOC_ALIGNMENT bytes, or if the last free
   block is only _ALLOC_ALIGNMENT big, will completely allocate this last free
   block for the array use.  In this latter case, the size of the array doesn't
   actually grow since the last free block is absorbed into the overhead (array)
   block.
*/
...
/*
        ...
        by one, and decreasing the array size by one.  If the end pointer
   changes in value by moving over an alignment boundary, the last free block
   implicitly grows in size by _ALLOC_ALIGNMENT bytes.
        ...
        block after the last allocated block.  However this has the effect that
   the array size does not shrink since you now need an extra pointer to point
        to the newly created free block.  But if the end pointer is currently
   taking up the space for the newly created free block, you've got a problem.

        To get around the problem of the end pointer sitting in the newly
   created free block, the erase function simply does not create the new free
   block, moves all of the pointers up by one, but has the end pointer continue
   to point to the old overhead block.  I.e. the overhead block does not shrink
   in size, and thus the last allocated block does not grow in size.

        If the end pointer already does not sit in memory big enough to create a
   free block (_ALLOC_ALIGNMENT bytes), then by erasing a pointer you make room
   for a new pointer to this new free block.  In this case the size of the array
   does not actually decrease.  The ith pointer is deleted, and simulateously a
   new pointer to the new last free block is created.  The end pointer doesn't
   move but is reset to point to itself, instead of into the next lowest aligned
   block. In this case, the end pointer will always be on an aligned boundary,
   but the align_down macro is used anyway just for consistency.
*/
...
/*
        static void erase2(unsigned char** e, unsigned char** i, size_t*
   nblocks)

        The erase2 function is very similar to the erase function.  It erases
   the pointers at i and at i-1.  It has the same issues as erase does
   concerning the situation when the last non-overhead block is allocated.  When
   it adds a pointer to point to a newly created last free block, erase2 only
   decrements the array size by 1.

        When _ALLOC_ALIGNMENT is 1, one of the possibilities is eliminated for
   erase2. Since you're always removing two elements, you always have room to
   insert a new pointer to the last free block by only removing one element.
   The if statement for this case is cast as asking if an unsigned type is less
   than 0, which of course can't happen.  It is hoped that the compiler will
...
/*
        static unsigned char** lower_bound(unsigned char** first, unsigned
   char** last, unsigned char* value)

        lower_bound is a binary searching function lifted from EWL C++.  Care
   has been taken to use > as the comparison instead of < since the array is
   stored in reverse order.  This function searches for value in the range
   [first, last) and returns an unsigned char** pointing to that value.
*/
...
/*
        ...
        The default name for this function is __allocate_size.  This function
   returns the size of an already allocated memory block.

        Precondition: ptr is non-null and has been returned from __allocate (or
   malloc, calloc, or realloc), and has not been deallocated (by free or
   realloc).

        Postcondition: The returned value indicates that the client can write
   arbitrary information to the range [(char*)vp, (char*)vp + returned_value)
   without fear of corrupting the heap.

        The bounds of the array are determined, and then lower_bound is used to
   search for the client supplied pointer.  Once found the size of the block is
   found by subtracting the next highest pointer from this pointer (remember the
   array is stored in reverse).

        In debug mode, asserts check that the client supplied pointer is in the
   array, and is allocated.
*/
...
/*
        ...
        The default name for this function is __allocate.  This function
   attempts to allocate a block of memory at least as big as size_requested. If
...
void *_EWL_CDECL _EWL_ALLOCATE(size_t size_requested, size_t *size_received) {
  ...
  if (!defaultheapinitialized)
    init_default_heap();
  if (size_requested && size_requested < (size_t)(-1) - _ALLOC_ALIGNMENT &&
      defaultheapinitialized) {
...
/*
        int _EWL_ALLOCATE_RESIZE(void* vp, size_t size_requested, size_t*
   size_received)

        The default name for this function is __allocate_resize.  This function
   attempts to expand or shrink a memory block in place to size_requested bytes.
   If successful, the return value is non-zero, otherwise zero is returned. On
        success, *size_received (if size_received is non-null) holds the actual
   size of the memory block. On failure *size_received holds a hint indicating a
   suggested size_requested that might succeed in a future call.

        Precondition: ptr is non-null and has been returned from __allocate (or
   malloc, calloc, or realloc), and has not been deallocated (by free or
   realloc).

        Postcondition: The memory block pointed to by ptr is still valid and has
   not been altered. If non-zero has been returned, then the current size of the
   memory block (as reflected by __allocate_size(ptr)) will be greater than or
   equal to size_requested. Additionally if size_received is non-null, then the
   result of
        ...
        size_requested is written to *size_received which may succeed in a
   future call to __allocate_resize.

        If size_requested is less than the size of the memory block prior to the
   call, then the size of the block after the call must be less than the size of
   the memory block before the call in order for success to be reported.  If
        size_requested is 0, the memory block is not free'd, but reduced to a
   minimum size if possible.
*/

int _EWL_CDECL _EWL_ALLOCATE_RESIZE(void *vp, size_t size_requested,
                                    size_t *size_received) {
  uchar_t *pc = (uchar_t *)vp;
    ...
    if (size_requested > current_size) {
      if (!is_allocated(*(i - 1))) {
        ...
        if (i - 2 == e && (size_t)((uchar_t *)e - ptr(*e)) >= _ALLOC_ALIGNMENT)
          could_grow = 1;
        if (current_size + next_size + could_grow * _ALLOC_ALIGNMENT >=
            aligned_request) {
...
/*
        int _EWL_ALLOCATE_EXPAND(void* vp, size_t min_size, size_t
   preferred_size, size_t* size_received)

        The default name for this function is __allocate_expand.  The intention
   of this function is to expand a block of memory in place to preferred_size,
   but failing to do that, expand to min_size. Failure is only indicated (with a
   zero return) if the memory block can not be expanded to min_size. If the size
   of the memory block prior to calling this function is already greater than
   min_size, a successful return is assured. If the size of the memory block
   prior to calling this function is greater than preferred_size, an attempt
   will be made to shrink the block down to preferred size, but success will be
        returned whether or not the shrink was successful. If non-null,
   *size_received will either contain the size of the memory block after a
   successful return, or on failure a suggested preferred_size that may succeed
   in a future call.

        Precondition: ptr is non-null and has been returned from __allocate (or
   malloc, calloc, or realloc), and has not been deallocated (by free or
   realloc). min_size <= preferred_size.

        Postcondition: The memory block pointed to by ptr is still valid and has
   not been altered. If non-zero has been returned, then the current size of the
   memory block (as reflected by
        __allocate_size(ptr)) will be greater than or equal to min_size.
   Additionally if size_received is non-null, then the result of
   __allocate_size(ptr) will be written to *size_received.

        If zero is returned and if size_received is non-null then a suggested
   preferred_size is written to *size_received which may succeed in a future
   call to __allocate_expand.
*/

int _EWL_CDECL _EWL_ALLOCATE_EXPAND(void *vp, size_t min_size,
                                    size_t preferred_size,
                                    size_t *size_received) {
  int result = 0;
  size_t sr_backup;
 ...
 *	--------
 *		init_alloc			Initialize allocs' memory pool to be
 *fixed size and location init_default_heap	initialize the default heap
 *		malloc				Allocate memory
 *		calloc				Allocate and clear memory
 *		realloc
 *		free				Return memory block to allocs' memory
...
/*	These two consts describe how malloc goes to the OS for memory.  The
shipping
//	configuration is that malloc will ask for at least 64Kb at a time from
the OS
//	and if bigger sizes are requested, they will go to the OS in
increments of 4Kb.
*/
...
/*	For tiny allocations, fixed sized pools help significantly speed
allocation/deallocation.
//	You can reserve a pool for a small range of sizes.  The use of these
pools can be disabled
//	by setting _EWL_USE_FIX_MALLOC_POOLS to 0.  Use of fixed size pools
requires further
//	configuration below.  The current shipping configuration is:
//  1. Each pool will handle approx. 4000 bytes worth of requests before asking
for more memory.
//	2. There are 4 pool types.  Each type is responsible for a different
range of requests:
//		a.  0 - 12 bytes
//		b. 13 - 20 bytes
//		c. 21 - 36 bytes
//		d. 37 - 68 bytes
//		Requests for greater than 68 bytes go to the variable size
pools.
//	The number of types of pools is configurable below.  The range of
requests for each
...
static const mem_size fix_pool_alloc_size =
    4096u; /* Approx space for each pool. */
#if defined(__ALTIVEC__)
static const mem_size max_fix_pool_size =
    76u; /* Need the highest poolsize elsewhere, so kludge it up like this */
static const mem_size fix_pool_sizes[] = {
    4u, 12u, 28u, 44u, 60u, max_fix_pool_size}; /* Client space for each pool */
#else
static const mem_size max_fix_pool_size =
    68u; /* Need the highest poolsize elsewhere, so kludge it up like this */
#if defined(__GNUC__)
static const mem_size fix_pool_sizes[] = {
    4u, 12u, 20u, 36u, 52u, 68u}; /* Client space for each pool */
#else
static const mem_size fix_pool_sizes[] = {
    4u, 12u, 20u, 36u, 52u, max_fix_pool_size}; /* Client space for each pool */
#endif
#endif /* __ALTIVEC__ */
...
*/
static const mem_size num_fix_pools =
    sizeof(fix_pool_sizes) / sizeof(mem_size); /* Not configurable */
...
typedef struct SubBlock {
  ...
  /*	...
  //	mem_size size_;   // SubBlock Trailer, only used when SubBlock is not
  allocated
...
#if _EWL_USE_FIX_MALLOC_POOLS
#if defined(__GNUC__)
static const mem_size SubBlock_min_size =
    ((2u * sizeof(mem_size)) + 68u + 1u + (mem_size)(alignment - 1)) &
    (mem_size) ~(mem_size)(alignment - 1);
#else
static const mem_size SubBlock_min_size =
    (SubBlock_overhead + max_fix_pool_size + 1u + (mem_size)(alignment - 1)) &
    (mem_size) ~(mem_size)(alignment - 1);
#endif
#else
#if defined(__GNUC__)
static const mem_size SubBlock_min_size =
    ((2 * sizeof(mem_size)) + 3 * sizeof(mem_size) +
     (mem_size)(alignment - 1)) &
    (mem_size) ~(mem_size)(alignment - 1);
#else
static const mem_size SubBlock_min_size =
    (SubBlock_overhead + 3u * sizeof(mem_size) + (mem_size)(alignment - 1)) &
    (mem_size) ~(mem_size)(alignment - 1);
#endif
#endif /* _EWL_USE_FIX_MALLOC_POOLS */
...
static void SubBlock_construct(SubBlock *ths, mem_size size, Block *bp,
                               int_t prev_alloc, int_t this_alloc);
...
/* Note, there is a bit of information currently unused when alignment == 16
 * (0x80) */
...
MISRA_EXCEPTION_RULE_19_7()
#define align(size, algn)                                                      \
  ((size + (mem_size)(algn - 1)) & (mem_size) ~(mem_size)(algn - 1))

#if _EWL_USE_FIX_MALLOC_POOLS

/* return true for SubBlock, false for FixSubBlock */
#define classify(ptr)                                                          \
  (*(mem_size *)((char_t *)(ptr) - sizeof(mem_size)) & fix_var_flag)
...
#define SubBlock_block(ths)                                                    \
  (/*lint -e(960)*/ (Block *)((mem_size)((ths)->bp_) & ~fix_var_flag))
...
#define SubBlock_set_free(ths)                                                 \
  ...
  *(mem_size *)((char_t *)(ths) + this_size) &=                                \
      (mem_size) ~(mem_size)prev_alloc_flag;                                   \
...
#define SubBlock_from_pointer(ptr)                                             \
  ((SubBlock *)((char_t *)(ptr)-SubBlock_header_size))
...
#define Block_start(ths)                                                       \
  (*(SubBlock **)(((char_t *)(ths) + Block_size((ths))) - sizeof(mem_size)))
...
static SubBlock *Block_subBlock(Block *ths, mem_size size,
  ...
  if (st == 0) {
    ...
    if ((sb_size - size) >= SubBlock_min_size) {
      MISRA_EXCEPTION_RULE_1_2a()(void) SubBlock_split(sb, size);
    }
...
static void Block_report(Block *ths, int_t verbose) {
  SubBlock *st;
  SubBlock *end;
  int_t i;
  int_t prev_free;
...
static SubBlock *SubBlock_split(SubBlock *ths, mem_size sz) {
  mem_size origsize;
  int_t isfree;
...
static Block *link_new_block(__mem_pool_obj *pool_obj, mem_size size) {
  Block *bp;
...
static void *allocate_from_var_pools(__mem_pool_obj *pool_obj, mem_size size,
                                     size_t *size_received) {
  Block *bp;
  SubBlock *ptr;
  ...
  bp = (pool_obj->start_ != 0) ? pool_obj->start_
                               : link_new_block(pool_obj, size);
  if (bp == 0) {
    MISRA_EXCEPTION_RULE_14_7()
    return 0;
  }
...
static void *soft_allocate_from_var_pools(__mem_pool_obj *pool_obj,
                                          mem_size size, mem_size *max_size) {
  Block *bp;
  SubBlock *ptr;
...
static void FixBlock_construct(FixBlock *ths, FixBlock *prev, FixBlock *next,
                               mem_size index, FixSubBlock *chunk,
                               mem_size chunk_size);
...
#define FixSubBlock_client_space(ths)                                          \
  ((char_t *)(ths) + FixSubBlock_header_size)

#define FixSubBlock_size(ths) (FixBlock_client_size((ths)->block_))

#define FixSubBlock_from_pointer(ptr)                                          \
  ((FixSubBlock *)((char_t *)(ptr)-FixSubBlock_header_size))
_MISRA_RESTORE()
...
static void *allocate_from_fixed_pools(__mem_pool_obj *pool_obj, mem_size size,
                                       size_t *client_received) {
  mem_size i = 0u;
  FixSubBlock *p;
  FixStart *fs;
  ...
  if ((fs->head_ == 0) || (fs->head_->start_ == 0)) {
    ...
    while (n >= 10u) {
      ...
      newblock = (char_t *)soft_allocate_from_var_pools(
    ...
    if (newblock == 0) {
      ...
      n = nsave;
      for (;;) {
        size_requested = (n * (fix_pool_sizes[i] + FixSubBlock_overhead)) +
                         FixBlock_overhead;
        newblock = (char_t *)allocate_from_var_pools(pool_obj, size_requested,
                                                     &cr_backup);
        if (newblock != 0) {
          break;
        }
        n = cr_backup /
            ((fix_pool_sizes[i] + FixSubBlock_overhead) + FixBlock_overhead);
        if (n < 10u) {
          if (client_received) {
            *client_received = 0u;
          }
    ...
    FixBlock_construct((FixBlock *)newblock, fs->tail_, fs->head_, i,
                       (FixSubBlock *)(newblock + FixBlock_overhead),
                       size_received - FixBlock_overhead);
    fs->head_ = (FixBlock *)newblock;
  }
  p = fs->head_->start_;
...
static int_t _EWL_CDECL __pool_allocate_resize(const __mem_pool *pool,
                                               void *ptr, size_t size_requested,
                                               size_t *size_received) {
#pragma unused(pool)
  MISRA_QUIET_UNUSED_ARGS()
  ...
  if (classify(ptr)) { /* is var_block */
    ...
    if (size_requested >
        (MAX_MEM_SIZE -
         ((mem_size)alignment + SubBlock_overhead + Block_overhead +
          (mem_size)alignment + SubBlock_min_size))) {
      size_requested = MAX_MEM_SIZE - ((mem_size)alignment + SubBlock_overhead +
                                       Block_overhead + (mem_size)alignment +
                                       SubBlock_min_size);
    }
    ...
    if (SubBlock_is_free(next_sub) &&
        (sz <= (this_size + SubBlock_size(next_sub)))) {
    ...
    if (size_requested > orig_size) {
      if (size_received && (size_requested > this_size) &&
          SubBlock_is_free(next_sub)) {
        MISRA_EXCEPTION_RULE_10_1()
...
MISRA_EXCEPTION_RULE_16_7()
size_t _EWL_CDECL __msize(void *ptr) {
#if _EWL_USE_FIX_MALLOC_POOLS
  if (!classify(ptr)) {
    MISRA_EXCEPTION_RULE_10_1()
    MISRA_EXCEPTION_RULE_14_7()
...
static void *_EWL_CDECL __pool_allocate(__mem_pool *pool, size_t size_requested,
                                        size_t *size_received) {
  void *result;
  ...
  if (size_requested >
      (MAX_MEM_SIZE - ((mem_size)alignment + SubBlock_overhead +
                       Block_overhead + (mem_size)alignment))) {
    if (size_received) {
      MISRA_EXCEPTION_RULE_10_1()
      *size_received = MAX_MEM_SIZE - ((mem_size)alignment + SubBlock_overhead +
                                       Block_overhead + (mem_size)alignment);
    }
    MISRA_EXCEPTION_RULE_14_7()
    return 0;
...
void *_EWL_CDECL __pool_alloc(__mem_pool *pool, size_t size) {
...
int_t _EWL_CDECL _EWL_ALLOCATE_RESIZE(void *ptr, size_t size_requested,
                                      size_t *size_received) {
  int_t result;
  __begin_critical_region(malloc_pool_access);
  result = __pool_allocate_resize(get_malloc_pool(), ptr, size_requested,
                                  size_received);
  __end_critical_region(malloc_pool_access);
  return result;
}

int_t _EWL_CDECL _EWL_ALLOCATE_EXPAND(void *ptr, size_t min_size,
                                      size_t preferred_size,
                                      size_t *size_received) {
  int_t result = 0;
  size_t sr_backup;
  ...
  while (min_size <= preferred_size) {
    result =
        __pool_allocate_resize(get_malloc_pool(), ptr, preferred_size, srp);
    if (result) {
      break;
    }
    preferred_size = *srp;
...
void _EWL_CDECL __pool_free(__mem_pool *pool, void *ptr) {
  __mem_pool_obj *pool_obj;
#if _EWL_USE_FIX_MALLOC_POOLS
  mem_size size;
#endif /* _EWL_USE_FIX_MALLOC_POOLS */
...
void *_EWL_CDECL __pool_realloc(__mem_pool *pool, void *ptr, size_t size) {
...
void _EWL_CDECL __pool_free_all(__mem_pool *pool) {
...
void _EWL_CDECL __malloc_free_all(void) { __pool_free_all(get_malloc_pool()); }
 ...
 *
 ***************************************************************************/
int_t init_alloc(void *heap_ptr, size_t heap_size) {
  mem_size size;
  Block *bp;
  int_t result;
  __mem_pool_obj *pool_obj;
...
void *vec_malloc(size_t size) { return malloc(size); }

void *vec_calloc(size_t nmemb, size_t size) { return calloc(nmemb, size); }

void *vec_realloc(void *ptr, size_t size) { return realloc(ptr, size); }

void vec_free(void *ptr) { free(ptr); }

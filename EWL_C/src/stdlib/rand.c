 ...
 *	Note
 *	----
 *		This is exactly the function described in the standard, including
 *the assumption that RAND_MAX == 32767.
 */
...
int_t _EWL_CDECL rand(void) {
  ...
  _EWL_LOCALDATA(random_next) =
      (_EWL_LOCALDATA(random_next) * 1103515245u) + 12345u;
  res = (_EWL_LOCALDATA(random_next) >> 16) & 0x7FFFu;
  return (int_t)res;
}

void _EWL_CDECL srand(uint_t seed) { _EWL_LOCALDATA(random_next) = seed; }

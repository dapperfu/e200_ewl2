...
void *_EWL_CDECL bsearch_s(const void *key, const void *table_base,
                           rsize_t num_members, rsize_t member_size,
  ...
  if ((num_members > (rsize_t)RSIZE_MAX) ||
      ...
      (((num_members > (rsize_t)0UL) && (key == NULL)) ||
       (table_base == NULL) || (compare_members == NULL))) {
  ...
  c = compare_members(key, mp,
                      context); /* We have to make sure 'key' doesn't compare */
  ...
  if (c == 0) /* As long as we're comparing, if it happens to */
  {
    ...
    if (c < 0) /* all over again via the binary search.        */
    {
      MISRA_EXCEPTION_RULE_14_7()
      return (NULL);
    }

...
typedef enum __io_results { __no_io_error, __io_error, __io_EOF } __io_results;
...
/** FILENAME_MAX is a constant expression that is the size needed for an array
 ...
 *	char large enough to hold a temporary file name string generated by the
 *tmpnam function.
 *
 * 	Applies only when DISK IO is supported.
 */
 ...
 *	distinct values, suitable for use as the third argument to the fseek
 *function. The specified position is the beginning of the file if whence is
 *SEEK_SET, the current value of the file position indicator if SEEK_CUR, or

...
/* FP EXCEPTION FLAGS */
#define FE_DIVBYZERO                                                           \
  0x04000000u /* bit 5 from left of the FPSCR, !!don't forget 2 count bit 0!!  \
               */
...
/* FP EXCEPTION FLAGS */
/* NOTE: These are the FPSCR exception status bits, not the exception enable
 * bits */
 ...
 * exceptions represented by its argument. Returns zero if the excepts argument
 * is zero or if all the specified exceptions were successfully cleared.
 ...
 * the argument excepts in the object pointed to by the argument flagp. Returns
 * zero if the representation was successfully stored. Otherwise, it returns a
 * nonzero value.
 */
_EWL_IMP_EXP_C int _EWL_MATH_CDECL fegetexceptflag(fexcept_t *flagp,
                                                   int excepts) _EWL_CANT_THROW;

/** The fesetexceptflag function attempts to set the floating-point status flags
 *  indicated by the argument excepts to the states stored in the object pointed
 * to by flagp. The value of *flagp shall have been set by a previous call to
 *  fegetexceptflag whose second argument represented at least those
 * floating-point exceptions represented by the argument excepts. This function
 * does not raise floatingpoint exceptions, but only sets the state of the
 * flags. Returns zero if the excepts argument is zero or if all the specified
 * flags were successfully set to the appropriate state. Otherwise, it returns
 * a nonzero value.
 */
_EWL_IMP_EXP_C int _EWL_MATH_CDECL fesetexceptflag(const fexcept_t *flagp,
                                                   int excepts) _EWL_CANT_THROW;
 ...
 * F.7.6. Whether the feraiseexcept function additionally raises the
 * ‘‘inexact’’ floating-point exception whenever it raises the ‘‘overflow’’ or
 * ‘‘underflow’’ floating-point exception is implementation-defined. Returns
 * zero if the excepts argument is zero or if all the specified exceptions were
 ...
 *environment in the object pointed to by envp. Returns zero if the environment
 *was successfully stored. Otherwise, it returns a nonzero value.
 */
 ...
 *	by the object pointed to by envp.  The argument envp shall point to an
 *object set by a call to fegetenv or feholdenv, or equal a floating-point
 ...
 *floating-point exceptions. Returns zero if the environment was successfully
 ...
 *floating-point exceptions. The argument envp shall point to an object set by
 *a call to feholdexcept or fegetenv, or equal a floating-point macro. Returns
 *zero if all the actions were successfully carried out. Otherwise, it returns
 *a nonzero value.
 */
_EWL_IMP_EXP_C int _EWL_MATH_CDECL feupdateenv(const fenv_t *envp)
    _EWL_CANT_THROW;
 ...
 */
_EWL_IMP_EXP_C void _EWL_MATH_CDECL
fegetexceptflag(fexcept_t *flagp, int excepts) _EWL_CANT_THROW;
 ...
 */
_EWL_IMP_EXP_C void _EWL_MATH_CDECL
fesetexceptflag(const fexcept_t *flagp, int excepts) _EWL_CANT_THROW;
 ...
 */
_EWL_IMP_EXP_C void _EWL_MATH_CDECL fesetenv(const fenv_t *envp)
    _EWL_CANT_THROW;
 ...
 *represented by the object pointed to by envp, and then raises the saved
 *floating-point exceptions. The argument envp shall point to an object set by
 *a call to feholdexcept or fegetenv, or equal a floating-point macro.
 */
_EWL_IMP_EXP_C void _EWL_MATH_CDECL feupdateenv(const fenv_t *envp)
    _EWL_CANT_THROW;

#endif /* _EWL_C99_TC2_FENV */

/** The fetestexcept function determines which of a specified subset of the
 * floatingpoint exception flags are currently set. The excepts argument
 * specifies the floatingpoint status flags to be queried. Returns the value of
 * the bitwise OR of the floating-point exception macros corresponding to the
 ...
 *flags, and then installs a non-stop (continues on floating-point exceptions)
 *mode, if available, for all floating-point exceptions. Returns zero if and
 *only if non-stop floating-point exception handling was successfully
 *installed.
 */

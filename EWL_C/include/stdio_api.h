...
_EWL_IMP_EXP_C size_t _EWL_CDECL __fread(void *, size_t, size_t,
                                         FILE *) _EWL_CANT_THROW;
_EWL_IMP_EXP_C size_t _EWL_CDECL __fwrite(const void *, size_t, size_t,
                                          FILE *) _EWL_CANT_THROW;
...
#if _EWL_WIDE_CHAR
#define __getc(_file)                                                          \
  ((__fwide((FILE *)_file, -1) >= 0)                                           \
       ? EOF                                                                   \
       : ((_file)->buffer_len-- ? (int)*(_file)->buffer_ptr++                  \
                                : __std(__get_char)((FILE *)_file)))
#define __putc(_c, _file)                                                      \
  ((__fwide((FILE *)_file, -1) >= 0)                                           \
       ? EOF                                                                   \
       : ((_file)->buffer_len--                                                \
              ? (int)(*(_file)->buffer_ptr++ = (unsigned char)(_c))            \
              : __std(__put_char)(_c, (FILE *)_file)))
#else
#define __getc(_file)                                                          \
  ((_file)->buffer_len-- ? (int)*(_file)->buffer_ptr++                         \
                         : __std(__get_char)((FILE *)_file))
#define __putc(_c, _file)                                                      \
  ((_file)->buffer_len-- ? (int)(*(_file)->buffer_ptr++ = (unsigned char)(_c)) \
                         : __std(__put_char)(_c, (FILE *)_file))
#endif /* _EWL_WIDE_CHAR && _EWL_C99 */

#if _EWL_WFILEIO_AVAILABLE
_EWL_IMP_EXP_C FILE *_EWL_CDECL
__whandle_open(__file_handle handle, const wchar_t *mode) _EWL_CANT_THROW;
FILE *_EWL_CDECL __whandle_reopen(__file_handle handle, const wchar_t *mode,
                                  FILE *) _EWL_CANT_THROW;
...
#if _EWL_WIDE_CHAR
_EWL_IMP_EXP_C __std(size_t) _EWL_CDECL
    __fwrite(const void *, __std(size_t), __std(size_t),
             FILE *) _EWL_CANT_THROW;
#endif
...
int __sformatter(int(_EWL_CDECL *ReadProc)(void *a, int b, int c),
                 void *ReadProcArg, const char *format_str, va_list arg,
                 int is_secure);

int __pformatter(void *(_EWL_CDECL *WriteProc)(void *a, const char *b,
                                               size_t c),
                 void *WriteProcArg, const char *_EWL_RESTRICT format_str,
...
void *_EWL_CDECL __wStringWrite(void *, const wchar_t *,
                                size_t) _EWL_CANT_THROW;

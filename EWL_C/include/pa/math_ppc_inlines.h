...
extern _EWL_INLINE float _inv_sqrtf(float x) {
  if (x > 0.0f) {
    ...
    guess =
        .5f * guess *
        (3.0f - guess * guess * xd); /* now have 8  sig bits */
    guess =
        .5f * guess *
        (3.0f - guess * guess * xd); /* now have 16 sig bits */
    guess =
        .5f * guess *
        (3.0f - guess * guess * xd); /* now have >24 sig bits		*/
    return (float)guess;
  } else if (x)
    return NAN;
...
extern _EWL_INLINE float _inv_sqrtf(float x) {
  ...
  */
  ...
  const _UINT32 bit_mask =
      0x007fffff & (~(sizeof(__inv_sqrt_guess) >> 2) << bit_shift);
  ...
  {
    (*(_UINT32 *)&scaled_x) =
        0x3E800000 + ((*(_UINT32 *)&x) & 0x007fffff); /* scaled_x in [.25,.5) */
    (*(_UINT32 *)&guess) =
        __inv_sqrt_guess[(first_several_sig_bits_of_x >> bit_shift)];

  } else {
    (*(_UINT32 *)&scaled_x) =
        0x3f000000 +
        ((*(_INT32 *)&x) & 0x007fffffU); /* scaled_x in [. 5,1.0) */
    (*(_UINT32 *)&guess) =
        __inv_sqrt_guess2[(first_several_sig_bits_of_x >> bit_shift)];
  }
  ...
  /* we now reduce x to 2^^n*y  where y is in [.5,1) we then calculate
     sqrt(x)=sqrt(2^^n)*sqrt(y) where if n is even we simply shift the exponent
     of guess appropriately or if n is odd we shift and multiply by sqrt(2) if n
     > 0 and 1/sqrt(2) if n > 0
  */

  if (biased_exp > 0x3f000000)
    (*(_INT32 *)&guess) -=
        (((biased_exp - 0x3e800000) >> 1) &
         0xffbfffff); /* this subtracts off bias(127=0x3f80...)
...
#pragma mark-- sqrt, sqrtf --
  ...
  /*	Fast sqrt() and sqrtf() implementations using "frsqrte"
          intrinsic instruction (non-IEEE conformant due to precision and errno
     handling) */
...
extern _EWL_INLINE float sqrtf(float x) {
  ...
  if (x > 0.0f) {
    ...
    guess = _half * guess *
            (_three - guess * guess * xd); /* now have 12 sig bits */
    guess =
        _half * guess *
        (_three - guess * guess * xd); /* now have 24 sig bits */
...
extern _EWL_INLINE float sqrtf(float x) {
  ...
  if (x > 0.0f) {
    ...
    guess =
        _half * guess *
        (_three - guess * guess * xd); /* now have 32 sig bits */
    return (float)(xd * guess);
  } else if (x < 0.0)
    return NAN;
  else if (isnan(x))
...
extern _EWL_INLINE double sqrt(double x) {
  if (x > 0.0) {
    ...
    guess = .5 * guess *
            (3.0 - guess * guess * x); /* now have 8 sig bits */
    guess = .5 * guess *
            (3.0 - guess * guess * x); /* now have 16 sig bits */
    guess = .5 * guess *
...
#endif /*!defined(__PPCZen__) && !defined(__PPCe500__) &&                      \
          !defined(__PPCe500v2)*/
...
extern _EWL_INLINE double fabs(double x) { return __fabs(x); }
...
#if defined(__CWCC__) &&                                                       \
    (__option(e500_floatingpoint) || __option(e500v2_floatingpoint))

...
#if !defined(_EMBEDDED_WARRIOR_MALLOC) && !defined(_EWL_BAREBOARD_MALLOC) &&   \
    !defined(_EWL_CLASSIC_MALLOC) && !defined(_EWL_OS_DIRECT_MALLOC)
...
#define _EWL_TIME_T_IS_LOCALTIME 1
    ...
    ((!_EWL_CLOCK_T_AVAILABLE) || (!_EWL_TIME_T_AVAILABLE))
...
#if !_EWL_USES_SUN_MATH_LIB &&                                                 \
    (_EWL_USES_SUN_SP_MATH_LIB || _EWL_USES_SUN_DP_MATH_LIB)
#error                                                                         \
    "Macro _EWL_USES_SUN_SP_MATH_LIB and _EWL_USES_SUN_DP_MATH_LIB require _EWL_USES_SUN_MATH_LIB be true!"
#endif
...
/*
   This macro is set to MATH_ERRNO (the value 1) by default.  If a particular
   platform has different math requirements, it should set _EWL_MATH_ERRHANDLING
   to the appropriate value in the platform prefix file.  C99 requires that
   MATH_ERRHANDLING (and thus _EWL_MATH_ERRHANDLING) be a value of MATH_ERRNO
   (1), MATH_ERREXCEPT (2), or MATH_ERRNO | MATH_ERREXCEPT (3).  No other values
   are valid.
*/

#ifndef _EWL_MATH_ERRHANDLING
#ifndef _EWL_C99
#define _EWL_MATH_ERRHANDLING                                                  \
  3 /* Define to MATH_ERRNO | MATH_ERREXCEPT by default */
#else
...
/*	Misra EXCEPTIONS that are used in headers files, see also
 * <ewl_misra_types.h> */

#define _MISRA_QUIET_DO_WHILE_0() /*lint -e{717} allow "do { ... } while(0)"   \
                                   */

#define _MISRA_EXCEPTION_STD_TYPE()   /*lint -e(970) [MISRA 2004 Rule 6.3] Use \
                                         of modifier or type outside of a      \
                                         typedef */

...
int _EWL_CDECL __open_file(const char *name, __std(__file_modes) mode,
                           __std(__file_handle) * handle) _EWL_CANT_THROW;
...
int __read_file(__std(__file_handle) handle, unsigned char *buffer,
                __std(size_t) * count,
                __std(__ref_con) ref_con) _EWL_CANT_THROW;
int __write_file(__std(__file_handle) handle, unsigned char *buffer,
                 __std(size_t) * count,
                 __std(__ref_con) ref_con) _EWL_CANT_THROW;
int __position_file(__std(__file_handle) handle, __std(fpos_t) * position,
                    int mode, __std(__ref_con) ref_con) _EWL_CANT_THROW;
#else
int __read_file(__std(__file_handle) handle, unsigned char *buffer,
                __std(size_t) * count) _EWL_CANT_THROW;
int __write_file(__std(__file_handle) handle, unsigned char *buffer,
                 __std(size_t) * count) _EWL_CANT_THROW;
int __position_file(__std(__file_handle) handle, __std(fpos_t) * position,
                    int mode) _EWL_CANT_THROW;
#endif
...
int _EWL_CDECL __rename_file(const char *old_name,
                             const char *new_name) _EWL_CANT_THROW;

_EWL_IMP_EXP_C __std(FILE) * _EWL_CDECL
    __reopen(__std(FILE) * file) _EWL_CANT_THROW;
_EWL_IMP_EXP_C int _EWL_CDECL __get_file_modes(const char *mode,
                                               __std(__file_modes) *
                                                   modes) _EWL_CANT_THROW;
#endif /* _EWL_OS_DISK_FILE_SUPPORT */

#if _EWL_WFILEIO_AVAILABLE
int _EWL_CDECL __wopen_file(const wchar_t *name, __std(__file_modes) mode,
                            __std(__file_handle) * handle) _EWL_CANT_THROW;
...
int _EWL_CDECL __wrename_file(const wchar_t *old_name,
                              const wchar_t *new_name) _EWL_CANT_THROW;
_EWL_IMP_EXP_C int _EWL_CDECL __wget_file_modes(const wchar_t *mode,
                                                __std(__file_modes) *
                                                    modes) _EWL_CANT_THROW;

...
{
#if defined(__POWERPC__) && !defined(__VLE__)
...
loop:
#if __GNUC__
	asm	volatile("lwarx %[a0],0,%[p0];" : [a0] "=r" (a) : [p0] "r" (p));
...
loop:
#if __GNUC__
	asm	volatile("l: lwarx %[a0],0,%[p0];" : [a0] "=r" (a) : [p0] "r" (p));
	n_readers = a & max_readers_;
	if (a & write_entered_ || n_readers == max_readers_)
		return false;
	++n_readers;
...
#endif //__GNUC__
#else  // __POWERPC__
	private_lock lk(mut_, CodeWarrior::try_to_lock);

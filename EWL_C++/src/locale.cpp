...
extern "C" void destroy_locale_mutex();

CodeWarrior::recursive_mutex&
get_locale_mutex_impl(bool init)
{
	static union
	{
		double dummy1;
		char dummy2[sizeof(CodeWarrior::recursive_mutex)];
	} v;
	static CodeWarrior::recursive_mutex* mut = 0;
	switch (init)
	{
	case false:
		if (mut)
		...
		return (CodeWarrior::recursive_mutex&)v.dummy1;
	case true:
		if (!mut)
		{
			mut = new (v.dummy2) CodeWarrior::recursive_mutex;
			atexit(destroy_locale_mutex);
		}
	}
	return *mut;
}

void destroy_locale_mutex()
{
	get_locale_mutex_impl(false);
}

void init_locale_mutex()
{
	get_locale_mutex_impl(true);
}

CodeWarrior::once_flag locale_init_flag = _EWL_THREAD_ONCE_INIT;

}

_EWL_IMP_EXP_CPP CodeWarrior::recursive_mutex& get_locale_mutex();

CodeWarrior::recursive_mutex&
get_locale_mutex()
{
	CodeWarrior::call_once(init_locale_mutex, locale_init_flag);
	return get_locale_mutex_impl(true);
}
...
{
	CodeWarrior::recursive_mutex::scoped_lock lock(get_locale_mutex());
	facets_ = rhs.facets_;
	name_ = rhs.name_;
...
__locale_imp::~__locale_imp() _EWL_NO_THROW
{
	CodeWarrior::recursive_mutex::scoped_lock lock(get_locale_mutex());
	size_t end = facets_.size();
	for (size_t i = 0; i < end; ++i)
...
void
locale::id::init_get(const type_info& ti)
{
	CodeWarrior::recursive_mutex::scoped_lock lock(get_locale_mutex());
	if (id_ == 0)
	{
		static CodeWarrior::hash_map<string, size_t> facet_id_map;
		size_t& id = facet_id_map[ti.name()];
		if (id == 0)
			id = ++id_count_s;
		id_ = id;
	}
...
#if !defined(_EWL_USING_EWL_C) || defined(_EWL_ENUM_ALWAYS_INT) || _EMBEDDED_WARRIOR_CTYPE
...
#else  // !defined(_EWL_USING_EWL_C) || defined(_EWL_ENUM_ALWAYS_INT)

static CodeWarrior::compile_assert<sizeof(_STD::ctype_base::mask) == sizeof(__ctype_mapC[0])> ctype_base_mask_not_compatible_with_c_lib;

#endif  // !defined(_EWL_USING_EWL_C) || defined(_EWL_ENUM_ALWAYS_INT)
	...
	char* to, char* to_limit, char*& to_next) const
{
	for (from_next = from, to_next = to; from_next < from_end && to_next < to_limit - (sizeof(wchar_t)-1); ++from_next, to_next += sizeof(wchar_t))
	{
		...
		for (unsigned i = 0; i < sizeof(wchar_t); ++i, ++p)
			to_next[i] = *p;
	}
	...
	wchar_t* to, wchar_t* to_limit, wchar_t*& to_next) const
{
...
#ifdef _EWL_EXTENDED_PRECISION_OUTP
...
#endif  // _EWL_EXTENDED_PRECISION_OUTP
...
#ifndef _EWL_EXTENDED_PRECISION_OUTP
	#ifdef _EWL_USING_EWL_C
		decimal d;
		decform form;
...
#else  // _EWL_EXTENDED_PRECISION_OUTP
		if (x > 0)
		{
			int exp;
	...
	_BCD::to_string(int precision, int& exponent) const
	{
		...
		exponent = exp_;
		if ((unsigned)precision < mantissa_.size())
		{
	...
	_BCD::to_long_double() const
	{
		...
		#ifdef _EWL_USING_EWL_C
			decimal d = {0, 0, 0, {0, ""}};
			d.sig.length = min<unsigned char>((unsigned char)decform_digits_, (unsigned char)mantissa_.size());

...
/*
        ...
        4.  There are two "traits" classes which specify whether the single
   object form of delete should be used, or the array form.  The default is
   single.
*/
...
// rebind members in _Single and _Array aid in auto_ptr conversions from
// auto_ptr<X> to auto_ptr<Y>.  The _Array rebind member uses T on purpose to
// prohibit such conversions.
...
public:
  ...
  operator=(const _RefCountedPtr<T, traits> &rhs) _EWL_NO_THROW;
...
template <class T, class traits>
inline _RefCountedPtr<T, traits>::_RefCountedPtr(
    const _RefCountedPtr<T, traits> &other) _EWL_NO_THROW
    : ptr_(other.ptr_),
...
template <class T, class traits>
_RefCountedPtr<T, traits> &_RefCountedPtr<T, traits>::
operator=(const _RefCountedPtr<T, traits> &rhs) _EWL_NO_THROW {
  if (ptr_ != rhs.ptr_) {
    T *ptr = rhs.ptr_;
...
template <class T, class traits>
inline T *_RefCountedPtr<T, traits>::operator->() _EWL_NO_THROW {
  return ptr_;
}

template <class T, class traits>
inline const T *_RefCountedPtr<T, traits>::operator->() const _EWL_NO_THROW {
  return ptr_;
}
...
template <class T, class traits>
inline _RefCountedPtr<T, traits>::operator const T *() const _EWL_NO_THROW {
  return ptr_;
}

template <class T, class traits>
inline bool _RefCountedPtr<T, traits>::isNonUnique() const _EWL_NO_THROW {
  if (refCount_ == 0)
    return false;
  return *refCount_ != 1;
}

...
typedef struct ex_destroylocalpointer {
  exaction_type action; //	EXACTION_DESTROYLOCALPOINTER
                        ...
                        /*  +--------------------------------+  */
  unsigned char
      dlp_field; //	regpointer	0 bit (1 -> object pointer is in a GPR)
                 //	unused 		1:7 (reserved for future use)
...
typedef struct ex_destroymember {
  exaction_type action; //	EXACTION_DESTROYMEMBER or EXACTION_DESTROYBASE
                        ...
                        /*  +--------------------------------+  */
  unsigned char
      dm_field;    //	regpointer	0 bit (1 -> object pointer is in a GPR)
                   //	unused 		1:7 (reserved for future use)
...
typedef struct ex_destroymembercond {
  exaction_type action; //	EXACTION_DESTROYMEMBERCOND
                        ...
                        /*  +--------------------------------+	*/
  unsigned char
      dmc_field;   //	regcond 	0 (1 -> condition flag is in a GPR)
...
typedef struct ex_destroymemberarray {
  exaction_type action; //	EXACTION_DESTROYMEMBERARRAY
                        ...
                        /*  +--------------------------------+  */
  unsigned char
      dma_field;     //	regpointer	0 bit (1 -> object pointer is in a GPR)
                     //	unused 		1:7 (reserved for future use)
  short objectptr;   //	16-bit frame-relative offset or GPR# of object pointer
  long offset;       //	offset of member in complete object
...
typedef struct ex_deletepointercond {
  exaction_type action; //	EXACTION_DELETEPOINTERCOND
                        ...
                        /*  +--------------------------------+	*/
  unsigned char
      dpc_field;    //	regcond 	0 (1 -> condition flag is in a GPR)
                    //	regpointer 	1 (1 -> object pointer is in a GPR)
                    //	unused	 	2:7 (reserved for future use)
  short cond;       //	16-bit frame-relative offset or GPR# of condition flag
  short objectptr;  //	16-bit frame-relative offset or GPR# of object pointer
...
typedef struct ex_catchblock {
  ...
  unsigned short
      catch_pcoffset; //	16-bit function-relative offset to catch label
...
typedef struct ex_destroymember_32 {
  exaction_type
      action; //	EXACTION_DESTROYMEMBER_32 or EXACTION_DESTROYBASE_32
              /*   0           1                  7   */
              /*  +--------------------------------+  */
              /*  | regpointer | unused            |  */
              /*  +--------------------------------+  */
...
typedef struct ex_destroybasevtt {
  exaction_type action; //	EXACTION_DESTROYBASEVTTOBJECT or
                        //EXACTION_DESTROYBASEVTTPOINTER
  unsigned char
      dm_field;   //	regpointer	0 bit (1 -> object pointer is in a GPR)
                  //	vttglobal	1 bit (1 -> vtt is global object
                  //	vttreg		2 bit (1 -> vtt is local register
                  //pointer
  short unused2;  //
  long objectptr; //	register, absolute, 32-bit frame-relative offset to
                  //object containing pointer
  long offset;    //	offset of member in complete object
  long vtt; //	register, absolute, 32-bit frame-relative offset to VTT object
  long vttoffset; //	offset in vtt to use
  void *dtor;     //	absolute pointer to TVector of destructor
} ex_destroybasevtt;
#define ex_destroybasevtt_MakeField(regpointer, vttglobal, vttreg)             \
  ((((regpointer) << 7)) | (((vttglobal) << 6)) | (((vttreg) << 5)))
...
typedef struct ex_destroyvla {
  exaction_type action; //	EXACTION_DESTROYVLA
                        ...
                        /*  +--------------------------------+  */
  unsigned char
      dma_field; //	regpointer	0 bit (1 -> array pointer is in a GPR)
...
#define ex_destroyvla_MakeField(regpointer, regsize)                           \
  (((regpointer) << 7) | ((regsize) << 6))
...
typedef struct ExceptionTableSmall {
  ...
  /*  +-----------------------------------------------------------+	*/
  unsigned short
      et_field; //	savedGPRs 	0:4 (# of saved GPRs (rN-r31))
  ...
  //	unsigned short	end = 0;			//	(marks end of range
  //list)
  //	unsigned long	actions[];			//	arbitrary action
  //data
} ExceptionTableSmall;
...
typedef struct ExceptionTableLarge {
  ...
  //  unsigned long   start = 0;			//  (marks end of range
  //  list)
  //	unsigned short	size = 0;			//	(marks end of range
  //list)
  //	unsigned long	actions[];			//	arbitrary action
  //data
} ExceptionTableLarge;
...
#define ET_MakeField(savedGPRs, savedFPRs, savedCR, hasframeptr, isLarge)      \
  (((savedGPRs) << 11) | ((savedFPRs & 0x001f) << 6) |                         \
   ((savedCR & 0x0001) << 5) | ((hasframeptr & 0x0001) << 4) |                 \
   ((isLarge & 1) << 3))

#if __SPE__ || defined(__PPCGEKKO__)
#define ET_AddElfHasVector(field, hasVector) ((field) | ((hasVector & 1) << 1))
...
#if __ALTIVEC__
#define ET_AddHasVector(field, hasVector) ((field) | ((hasVector & 1) << 2))
...
typedef struct ExceptionTableIndex {
  unsigned long functionoffset; //	code-relative address of function
                                ...
                                /*  +--------------------------------+  */
  unsigned long
      eti_field; //	directstore 		0 (1 -> 4 byte exception table is
                 //in 'exceptionoffset') 	functionsize 	1:31 (length
                 //of function)
...
#define ETI_MakeField(direct, fsize)                                           \

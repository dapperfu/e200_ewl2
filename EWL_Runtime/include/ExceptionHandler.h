 ...
 *	ThrowContext
 *
 *	This structure records all the necessary information available at the
 *point where an exception is thrown, and is also used to step back through the
 *pending stack frames as we look for a catch-block.
 *
 */
...
typedef struct ExceptionInfo {
  ...
  ExceptionTableIndex *exception_table_start; /*	pointer to start of
                                                 exception table index	*/
  ExceptionTableIndex
      *exception_table_end;   /*	pointer to end of exception table index	*/
 ...
 *
 *	After the target-specific implementation of __throw() saves all of the
 *relevant throw information in a ThrowContext, it calls this target-independent
 *portion to do all of the necessary C++ exception-handling.
 *
 */
...
bool __uncaught_exception_helper(ThrowContext *context);
_EWL_END_EXTERN_C

...
typedef struct CatchInfo {
  ...
  void *dtor;        //	pointer to complete exception object's destructor
                     //function (0: no destructor)
...
_EWL_IMP_EXP_RUNTIME extern void __throw(char *throwtype, void *location,
                                         void *dtor);
_EWL_IMP_EXP_RUNTIME extern void __rethrow(void);
_EWL_IMP_EXP_RUNTIME extern char __throw_catch_compare(const char *throwtype,
...
__declspec(private_extern) int __register_mach_sorted(char *code_start,
                                                      char *code_end,
                                                      char *exception_start,
                                                      char *exception_end);
__declspec(private_extern) int __register_mach_header(
    const struct mach_header *header, unsigned long vmaddr_slide);
__declspec(private_extern) void __unregister_mach_header(int fragmentID);
#else
_EWL_IMP_EXP_RUNTIME extern int
__register_fragment(char *code_start, char *code_end, char *data_start,
                    char *data_end, char *exception_start, char *exception_end,
                    char *TOC);
_EWL_IMP_EXP_RUNTIME extern void __unregister_fragment(int fragmentID);
#endif /* __MACH__ */
#endif /* __PPC_EABI__ */

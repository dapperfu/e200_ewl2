...
extern void __init_cpp(void) {
   ...
   */
  for (constructor = __CTOR_LIST__; constructor != __CTOR_END__;
       constructor++) {
    (*constructor)();
  }

  for (constructor = __preinit_array_start; constructor != __preinit_array_end;
       constructor++) {
    (*constructor)();
  }

  for (constructor = __init_array_start; constructor != __init_array_end;
       constructor++) {
    (*constructor)();
  }
}
...
extern void __fini_cpp(void) {
  ...
  for (destructor = __fini_array_end; destructor != __fini_array_start;
       --destructor) {
    (*(destructor - 1))();
  }
}

...
DESCRIPTION
        ...
        If hardware initialization and pre-main user initialization are
required, copy this file to your project directory and customize it (instead of
        ...
        void _ExitProcess(void)
                ...
                This function simply stalls the debugger.  You may want to
rewrite this function if you are using an OS.
        
        abort and exit
 
        In order to correctly implement the required startup/termination
sequence for C and C++ programs, we need to have an exit() routine that can be
called by the program startup code. The exit() routine is supposed to
        ...
        We don't, however, want to require the ANSI C library for every
CodeWarrior program, since it drags in lots of code that may not be needed.
 
        Instead we provide a dummy exit() function which simply calls the
destructors and terminates the program. We assume that any program which uses
atexit() or <stdio.h> and which requires those cleanup behaviors will have
linked with the ANSI C library, whose definition of exit() will override the one
here.
 
        We similarly define a dummy abort() function (which is called by the
default terminate() handler).
 
        Programs which rely on the proper ANSI C/C++ behavior must use the ANSI
C library, and order it in the CodeWarrior project or command-line so that its
definitions supersede these definitions in the runtime support library.
...
HISTORY
        ...
        97 JUN 26 MEA	Made C and C++ versions of this file.  Added abort
and exit. 97 JUL 17 SCM	Customized for MPC821 ADS board. 97 JUL 20 MEA
Changed __exit to _ExitProcess so as not to conflict with EWL. _ExitProcess
added to this file; removed form __start.c.
        99 JAN 22 MEA	Removed exception handing to
__init_cpp_exceptions.c for use with 2.2.2 compiler (post Release 4).  Put
prototypes for exit and abort within extern "C" to avoid name-mangling.
...
asm void __init_user(void) {
   ...
   */
    ...
    !defined(DO_NOT_LOAD_CTORS_DTORS)
          bl __init_cpp
#endif
...
extern void abort(void) { _ExitProcess(); }
 ...
 *	__flush_cache
 *
 *	For the given memory range, forces all modified data cache contents to
 *be written to memory and invalidates the associated instruction cache entries.
 *
 */
/****************************************************************************/
 ...
 */
/* __init_registers and __init_data will overload weak versions found in
 * __start.c	*/
 ...
 *
 *	Utility function to get difference between runtime and linktime
 *addresses.
 *
 */
/***************************************************************************/
static asm int __get_runtime_linktime_delta(void) {
  nofralloc
       ...
       */
          /* _SDA_BASE_ but forces the linker to treat it as absolutetly
             addressed 	*/
              /* and not to substitute r13 for r0.  We therefore end up with r3
                 exactly 	*/
                  /* as calculated at linktime.
                   */
                      addis r3,
      r0, _ABS_SDA_BASE_ @ha addi r3, r3,
      _ABS_SDA_BASE_ @l
  ...
  nofralloc
       ...
       */
          ...
          mflr r0
              ...
              /* If runtime and linktime addresses are the same, no fixup is
                 necessary.	*/
                  bl __get_runtime_linktime_delta cmpli cr0,
      r3,
      0 beq no_fixup

          /* Initialize runtime addresses of the fixup table.  _f_mwerks_fixup
             is the	*/
              /* beginning of the table and _e_mwerks_fixup is the end.  Note
                 that r0 	*/
                  /* will get converted to r13 or r2 by the linker.
                   */
                      addis r5,
      r0, _f_mwerks_fixup @ha addis r4, r0, _e_mwerks_fixup @ha addi r5, r5,
      _f_mwerks_fixup @l addi r4, r4,
      _e_mwerks_fixup @l

          /* Compute number of entries ((_e_mwerks_fixup - _f_mwerks_fixup) /
             4).		*/
              subf r6,
      r5,
      r4
      ...
      r3
           ...
           */
              stw r7,
      0(r6)bdnz fixup_loop no_fixup : mtlr r0 blr
}
 ...
 *
 *	Note: this function is guaranteed to not reference any memory; the
 *memory controller may not be initialized.
 *
  ...
  nofralloc /* see above on usage of nofralloc */
      ...
      mflr r0
           ...
           */
              /* _SDA_BASE_ but forces the linker to treat it as absolutetly
                 addressed 	*/
                  /* and not to substitute r13 for r0.  We therefore end up with
                     r13 exactly 	*/
                      /* as calculated at linktime.
                       */
                          addis r13,
      r0, _ABS_SDA_BASE_ @ha addi r13, r13,
      _ABS_SDA_BASE_ @l

          /* _ABS_SDA2_BASE_, generated by the linker, is the same value as
           */
              /* _SDA2_BASE_ but forces the linker to treat it as absolutetly
                 addressed 	*/
                  /* and not to substitute r2 for r0.  We therefore end up with
                     r2 exactly 	*/
                      /* as calculated at linktime.
                       */
                          addis r2,
      r0, _ABS_SDA2_BASE_ @ha addi r2, r2,
      _ABS_SDA2_BASE_ @l

          /* Grab the runtime address of __get_linktime_address.  The bcl will
             put	*/
              /* __get_linktime_address into LR. */
              bcl 20,
      31,
      __get_linktime_address entry
          __get_linktime_address
              /* Move runtime address of __get_linktime_address from LR to r4.
               */
                  mflr
                      r4

                          /* Grab the linktime address of
                             __get_linktime_address.  Note that r0 will	*/
                              /* get converted to r13 or r2 by the linker but
                                 because r13 and r2 may not	*/
                                  /* reflect the their runtime values yet, r3
                                     might not equal r4.
                                   */
                                      addis r3,
      r0, __get_linktime_address @ha addi r3, r3,
      __get_linktime_address @l
      ...
      0 beq no_fixup
          ...
          /* Add the delta into small data pointers so they will reflect the
             current 	*/
      ...
      addis r1,
      r0,
      _stack_addr @ha addi r1,
      r1,
      _stack_addr @l mtlr r0
...
extern void __init_data(void) {
  ...
  /* for sda pic pid, if ram_end and ram_addr are swapped so that ram_end <
   * ram_addr, 		*/
  /* then segment is internal, otherwise it is external.  for eabi, addresses
   * are external 	*/

  dci = _pic_rom_copy_info;
  while (1) {
    if (dci->rom_addr == 0 && dci->ram_addr == 0 && dci->ram_end == 0)
      break;
  ...
  while (1) {
    if (bii->ram_addr == 0 && bii->ram_end == 0)
      break;
 ...
 *	returnaddr	address within a code segment
 *	ex_start	address of variable to hold address of exception table
 *info
 *	ex_end		address of variable to hold end address of exception
 *table info
 *
 *	we now overload the function in the event that the user wants pic pid
 *
 */
/***************************************************************************/
extern int __find_exception_addresses(void *info, char *returnaddr,
                                      void **ex_start, void **ex_end) {
  ...
  while (1) {
    if (eti_info->code_end - eti_info->code_start == 0)
      break;
    ...
    if ((unsigned char *)returnaddr >= start &&
        (unsigned char *)returnaddr < end) {

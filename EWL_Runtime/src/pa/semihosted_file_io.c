...
/* The _temp_file_info struct contains enough information to identify a
 * temporary file */
/* based on its __file_handle, and enough information to delete the temporary
 * file when */
...
static int translate_msl_to_semihost_mode(__file_modes modes) {
  ...
  case __read_mode:
  case __write_mode:
  case (__write_mode | __append_mode):
    iomode_offset = 0;
    break;
  case __read_write_mode:
  case (__read_write_mode | __append_mode):
    iomode_offset = 2;
    break;
  }
...
int sys_open(register void *op) _EWL_WEAK _EWL_DO_NOT_INLINE;
int sys_open(register void *op) {
  // r0 = 5 (OsOpen)
  // r3 = name of file to open
  // r4 = flags for file status and access mode
  int value = -1;

#ifdef __VLE__
  __asm volatile(
      "se_li 0, %1; se_mr 3, %2; se_mr 4, %3; se_mr 5, %4; se_sc; se_mr %0, 0"
      : "=kregs"(value)
      : "i"(Semihosting_Open), "kregs"(((semihost_open_parms *)op)->name),
        "kregs"(((semihost_open_parms *)op)->semihost_mode),
        "kregs"(((semihost_open_parms *)op)->name_len)
      : "r0", "r3", "r4", "r5");
#endif
...
int open(const char *path, int access, ...) {
  ...
  if (access & O_RDONLY)
    modes.io_mode |= __read_mode;
  if (access & O_WRONLY)
    modes.io_mode |= __write_mode;
  if (access & O_RDWR)
    modes.io_mode |= __read_write_mode;
  if (access & O_APPEND)
    modes.io_mode |= __append_mode;

#if _EWL_TINY_FILE_IO
  if (access & O_CREAT) {
    modes.open_mode = 1;
    if (access & O_TRUNC)
      modes.open_mode = 2;
...
int sys_close(register unsigned long *fh) _EWL_WEAK _EWL_DO_NOT_INLINE;
int sys_close(register unsigned long *fh) {
  // r0 = 6 (OsClose)
...
#ifdef __VLE__
  __asm volatile("se_li 0, %1; se_mr 3, %2; se_sc; se_mr %0, 0"
                 : "=kregs"(value)
                 : "i"(Semihosting_Close), "kregs"(block[0])
                 : "r0", "r3");
#endif
...
int __close_file(__file_handle handle) {
  ...
  /* If the file was a temporary file, delete it and remove the tracking
   * information */
  if (info != NULL) {
...
int close(int h) { return __close_file(h); }
...
int fstat(int h, struct stat *buf) { return -1; }
...
int sys_write(register void *wp) _EWL_WEAK _EWL_DO_NOT_INLINE;
int sys_write(register void *wp) {
  // r0 = 4 (OsWrite)
  // r3 = file handle returned from system call open(...)
  // r4 = character buffer cast as void* pointer
  // r5 = number of bytes (nbyte) to write
  int value = 0;
  semihost_readwrite_parms *block = wp;

#ifdef __VLE__
  __asm volatile(
      "se_li 0, %1; se_mr 3, %2; se_mr 4, %3; se_mr 5, %4; se_sc; se_mr %0, 0"
      : "=kregs"(value)
      : "i"(Semihosting_Write), "kregs"(block->handle), "kregs"(block->data),
        "kregs"(block->count)
      : "r0", "r3", "r4", "r5");
#endif
 ...
 *
 */
int __write_file(__file_handle handle, unsigned char *buf, size_t *cnt) {
...
int write(int h, void *buf, unsigned len) {
  if (__write_file(h, (unsigned char *)buf, (size_t *)&len) == __no_io_error) {
    return (len);
  } else {
    return -1;
  }
...
int sys_read(register void *rp) _EWL_WEAK _EWL_DO_NOT_INLINE;
int sys_read(register void *rp) {
  // r0 = 3 (OsRead)
  // r3 = file handle returned from system call open(...)
  // r4 = character buffer cast as void* pointer
  ...
  __asm volatile(
      ...
      : "=kregs"(value)
      : "i"(Semihosting_Read),
        "kregs"(((semihost_readwrite_parms *)rp)->handle),
        "kregs"(((semihost_readwrite_parms *)rp)->data),
        "kregs"(((semihost_readwrite_parms *)rp)->count)
      : "r0", "r3", "r4", "r5");
#endif
 ...
 *
 */
int __read_file(__file_handle handle, unsigned char *buf, size_t *cnt) {
...
int read(int h, void *buf, unsigned len) {
  if (__read_file(h, (unsigned char *)buf, (size_t *)&len) == __no_io_error) {
    return (len);
  } else
    return -1;
}
 ...
 *
 */
int sys_seek(register void *p) _EWL_WEAK _EWL_DO_NOT_INLINE;
int sys_seek(register void *p) {
// r0 = 5 (OsOpen)
// r3 = name of file to open
// r4 = flags for file status and access mode
#pragma unused(p)
...
#ifdef __VLE__
  __asm volatile("e_li 0, %0" ::"i"(Semihosting_Seek) : "r0");
 ...
 *
 */
int sys_flen(register int *p) _EWL_WEAK _EWL_DO_NOT_INLINE;
int sys_flen(register int *p) { return -1; }

/*
 *	__position_file	-	Set up for host file position
 *
 */
int __position_file(__file_handle handle, unsigned long *offset, int mode) {
...
long lseek(int h, long offset, int whence) {
  return __position_file(h, (unsigned long *)&offset, whence);
}
 ...
 *
 */
int sys_tmpnam(register void *tnp) _EWL_WEAK _EWL_DO_NOT_INLINE;
int sys_tmpnam(register void *tnp) {}
 ...
 *
 */
int sys_remove(register void *p) _EWL_WEAK _EWL_DO_NOT_INLINE;
int sys_remove(register void *p) {}
 ...
 *
 */
int sys_rename(register void *p) _EWL_WEAK _EWL_DO_NOT_INLINE;
int sys_rename(register void *p) {}
...
/*
 * Semihost initialization - stack must be ready to execute C before this is
 * called
 *
 */
void __init_semihost(void) _EWL_WEAK;
void __init_semihost(void) {}

...
FILE
        ...
        $Date: 2013/01/08 21:20:03 $
        $Revision: 1.1.2.1 $
DESCRIPTION

        CodeWarrior PowerPC EABI Runtime Initialization.	 This file
provides the first entry point, "__start" which is used by the linker as a
        ...
        void __start(void)
                ...
                Main entry point for the PPC EABI program.	It will call
main().
...
REFERENCES
        ...
        [EABI95]	PowerPC Embedded Application Binary Interface,
                                32-bit Implementation.	Version 1.0,
Initial Release, 1/10/95.  Stephen Sobek, Motorola, and Kevin Burke, IBM.
...
NOTES
        ...
        Defined, but unused linker-generated symbols:
                _stack_size		Defined but not used yet.  For bounds
checking. _heap_size		Defined but not used yet.  For user heap. _ftext
Defined but not used.  Start of text section.
                _etext			Defined but not used.  Address
after end of text.
...
HISTORY
        ...
        97 FEB 19 LLY	Added __copy_rom function as a future placeholder for
                                        copying ROM-based data into RAM.
__start changed to
                                        __init_main for SDSMON07
        97 FEB 24 LLY	Test for zero length .bss and .sbss
        97 FEB 28 LLY	Converted to C code.  Added the rest of the EABI
sections
                                        (.sdata, .sbss, .sdata2)
        ...
        97 APR 16 LLY	Converted runtime init for PPC EABI tools 4/1/97.
                                        ...
                                        "__ppc_eabi_linker.h".  Register
initialization was moved into a function. 97 APR 19 LLY	Added comments
regarding safe usage of memory. 97 MAY 01 LLY	Added GRC's initial stack frame.
        ...
        97 DEC 7  MEA	Changed linker generated symbols to C type variables.
                                        Use _rom_copy_info and _bss_copy_info
instead of individual sections symbols. 97 DEC 8  MEA	Added section
pragma to put all code in this file into
                                        ".init"
        02 JAN 7  EJS   Preserve r3,r4,r5 and pass them to main() in case the
environment passes argc, argv, and envp
*/
/***************************************************************************/
...
SECTION_INIT extern void __start(int argc, char **argv,
                                 char **envp); /* primary entry point */
SECTION_INIT extern void exit(int);
...
asm void __start(register int argc, register char **argv,
                 register char **envp) {
  nofralloc /* MWERKS: explicitly no stack */
            /* frame allocation */

      mr r14,
      argc mr r15, argv mr r16,
      envp
...
#if __SPE__
              stwu r1,
      -16(r1) /* e500 required SP to always be 16-byte aligned */
#else
                  stwu r1,
      -8(r1) /* Decrement stack by 8 bytes, (write word)*/
#endif
      li r0,
      0x0000 /* load up r0 with 0x00000000 */
      stw r0,
      0(
          r1) /* SysVr4 EABI Supp indicated that initial back chain word should
                 be null */
      li r0,
      0xFFFF /* load up r0 with 0xFFFFFFFF */
      stw r0,
      4(r1) /* Make an illegal return address of 0xFFFFFFFF */
   ...
   */
          ...
          lis r3,
      __init_user @ha addi r3, r3,
      __init_user @l mtlr r3 blrl
#else
      bl __init_user
#endif
...
#if defined(USE_FAR_ADDRESSING_TO_TEXT_SECTION)
          lis r6,
      main @ha addi r6, r6, main @l mtlr r6 mr r3, r14 mr r4, r15 mr r5,
      r16 blrl
#else
          mr r3,
      r14 mr r4, r15 mr r5,
...
#if defined(USE_FAR_ADDRESSING_TO_TEXT_SECTION)
          lis r6,
      exit @ha addi r6, r6, exit @l mtlr r6 blrl
#else
          b exit
#endif
}
...
asm extern void __init_registers(void) {
  nofralloc /* see above on usage of nofralloc */
      ...
      lis r1,
      _stack_addr @ha /* _stack_addr is generated by linker */
          addi r1,
      r1,
      _stack_addr @l
           ...
           */
          lis r2,
      _SDA2_BASE_ @ha /* _SDA2_BASE_ is generated by linker */
          addi r2,
      r2,
      _SDA2_BASE_ @l

          lis r13,
      _SDA_BASE_ @ha /* _SDA_BASE_ is generated by linker */
          addi r13,
      r13,
      _SDA_BASE_ @l
#endif
...
extern void __init_data(void) {
  ...
  dci = _rom_copy_info;
  while (1) {
    if (dci->rom == 0 && dci->addr == 0 && dci->size == 0)
      break;

 ...
 */
/*				Design from 'Workplace Application Binary Interface
 * for						*/
/*				the PowerPC'.
 */
/*	Copyright.:	©Copyright 1993-2006 by Freescale Semiconductor, Inc.
 * All rights reserved.	*/
...
typedef enum {
  ...
  ,
  arg_SFPEDOUBLE =
      10 /* not treated like a arg_DOUBLEWORD */
#endif
} _va_arg_type;
...
void *__va_arg(va_list ap, _va_arg_type type) {
  ...
  int size = SIZEOF_GPR;    /* size of 'type'
                             */
  int inc =
      1;        /* num of registers to increment per type        */
  int even = 0; /* do we have a type that takes 2 registers? */
  int fpr_offset =
      0;                     /* where do the floating point registers live			*/
  ...
  if (rr < max) {
    ...
    *r =
        MAX_SAVED_REGS; /* if arg_DOUBLEWORD, make sure that gprs won't be used
                           again */
    addr = ap->input_arg_area;
    addr = (char *)ALIGN(addr, size);
    ap->input_arg_area = addr + size;
